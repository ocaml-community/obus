...
val send_message_sync : t -> ?raise_exn:bool -> send_message -> recv_message
  (** [send_message_sync connection raise_exn message] send a message
      over a DBus connection.

      If [raise_exn] is [true] (the default) and the reply is an
      error, then a [Error.DBus] will be raised. If it is [false] the
      error will just be returned.
...
(* `raw' version of the previous functions. They are used by
   autogenerated code. *)

type 'a reader = Header.recv -> buffer -> ptr -> 'a
type writer = buffer -> ptr -> ptr
...
(* Needed by the Cookie module *)
val read_values : bool -> Values.values reader
val write_values : Values.values -> Wire.byte_order -> writer
