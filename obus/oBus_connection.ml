...
let send_message_backend connection running header with_serial signature writer =
  running.outgoing <-
    (running.outgoing >>=
       fun (serial, buffer) ->
         ...
         let serial = Int32.succ serial in
           ...
           (* Write the message *)
           Lwt.try_bind
             (fun _ -> send_one_message connection
                { send_header = header;
                  send_serial = serial;
                  send_writer = writer;
                  send_signature = signature;
                  send_byte_order = OBus_info.native_byte_order } buffer)
             (fun buffer -> return (serial, buffer))
             (function
                | OBus_transport.Error _ as exn ->
                    set_crash connection fail (Lwt.fail exn) exn
                | exn -> fail exn))

(* Register a reply handler which expect a fixed signature *)
let register_reply_handler call_header w typ running serial =
  let reply_comb = OBus_comb.func_reply typ in
  let OBus_annot.Annot seq = OBus_comb.annot reply_comb in
  let reply_sig = OBus_annot.sequence_type_of_ext seq in
    running.reply_handlers <-
      (Serial_map.add serial
         ((fun header ctx sign ptr body ->
             ...
             if sign = reply_sig
             then wakeup w (header, snd (OBus_comb.reader reply_comb ctx ptr))
             else
    ...
    match header with
      ...
      | { typ = `Method_call(path, Some(interface), member) } as header ->
          begin match Interf_map.lookup interface running.service_handlers with
            | None ->
                send_exn connection header $ OBus_error.Unknown_method (sprintf "No such interface: %S" interface)
            | Some handler -> match handler header signature with
                | Mchr_no_such_method ->
                    send_exn connection header $ OBus_error.Unknown_method
                      (sprintf "Method %S with signature %S on interface %S doesn't exist"
                         member (string_of_signature signature) interface)
                | Mchr_no_such_object ->
                    send_exn connection header $ OBus_error.Failed (sprintf "No such object: %S" path)
      ...
      | { typ = `Method_call(path, None, member) } as header ->
          begin
            ...
            with
              ...
              | l ->
                  send_exn connection header $ OBus_error.Failed
                    (sprintf
