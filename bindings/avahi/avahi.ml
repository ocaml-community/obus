...
module Server = struct
  ...
  let entry_group_new = call "EntryGroupNew" << unit -> proxy >>
  let domain_browser_new = call "DomainBrowserNew" << int -> int -> string -> int -> uint -> proxy >>
  let service_type_browser_new = call "ServiceTypeBrowserNew" << int -> int -> string -> uint -> proxy >>
  let service_browser_new = call "ServiceBrowserNew" << int -> int -> string -> string -> uint -> proxy >>
  let service_resolver_new = call "ServiceResolverNew" << int -> int -> string -> string -> string -> int -> uint -> proxy >>
  let host_name_resolver_new = call "HostNameResolverNew" << int -> int -> string -> int -> uint -> proxy >>
  let address_resolver_new = call "AddressResolverNew" << int -> int -> string -> uint -> proxy >>
  let record_browser_new = call "RecordBrowserNew" << int -> int -> string -> uint16 -> uint16 -> uint -> proxy >>
end
