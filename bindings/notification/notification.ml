...
let notifications : notification Id_map.t ref Peer_map.t ref =
  ref Peer_map.empty
...
let get_server_information () =
  ...
  let%lwt name, vendor, version, spec_version =
    OBus_method.call m_GetServerInformation proxy ()
  in
  return
    {
      server_name = name;
...
let notify proxy ~app_name ~id ~icon ~summary ~body ~actions ~hints ~timeout =
  let%lwt context, return_id =
    OBus_method.call_with_context m_Notify proxy
      (app_name, id, icon, summary, body, actions, hints, Int32.of_int timeout)
  in
  return (OBus_context.sender context, return_id)

let close_notification proxy id = OBus_method.call m_CloseNotification proxy id

let s_NotificationClosed =
  OBus_member.Signal.make ~interface:"org.freedesktop.Notifications"
    ~member:"NotificationClosed"
    ~args:(arg2 (None, C.basic_uint32) (None, C.basic_uint32))
    ~annotations:[]

let notification_closed proxy = OBus_signal.make s_NotificationClosed proxy

let s_ActionInvoked =
  OBus_member.Signal.make ~interface:"org.freedesktop.Notifications"
    ~member:"ActionInvoked"
    ~args:(arg2 (None, C.basic_uint32) (None, C.basic_string))
    ~annotations:[]

let action_invoked proxy = OBus_signal.make s_ActionInvoked proxy
...
let init_callbacks =
  lazy
    (let%lwt bus = OBus_bus.session () in
     ...
     let anonymous_proxy =
       {
         OBus_proxy.peer = OBus_peer.anonymous bus;
         OBus_proxy.path = server_path;
       }
     in
     let%lwt event =
       OBus_signal.connect
         (OBus_signal.map_with_context
            (fun context (id, reason) ->
              (OBus_context.sender context, id, reason))
     ...
     E.keep
       (E.map_p
          (fun (peer, id, reason) ->
            match
              try Some (Peer_map.find peer !notifications)
              with Not_found -> None
            with
            | None -> return ()
            | Some m -> (
...
let close n =
  ...
  if not notif.notif_deleted then (
...
let notify ?(app_name = !app_name) ?desktop_entry ?replace ?(icon = "") ?image
    ~summary ?(body = "") ?(actions = []) ?urgency ?category ?sound_file
    ?suppress_sound ?pos ?(hints = []) ?(timeout = -1) () =
  let desktop_entry =
    match desktop_entry with None -> !default_desktop_entry | x -> x
  in
  (*** Creation of hints ***)
  let make_hint name x f =
    match x with Some x -> Some (name, f x) | None -> None
  in
  let hints =
    filter_opt
      [
        make_hint "desktop-entry" desktop_entry V.basic_string;
        make_hint "image_data" image (fun image ->
            V.structure
              [
        ...
        make_hint "urgency" urgency (fun urgency ->
            V.basic_int32
              (match urgency with `Low -> 0l | `Normal -> 1l | `Critical -> 2l));
  ...
  let%lwt peer, id =
    notify daemon ~app_name
      ~id:(match replace with Some n -> n.id | None -> 0l)
  ...
  let notif =
    {
      ...
      notif_action =
        (fun action ->
          wakeup wakener
            (try List.assoc action actions_map with Not_found -> `Default));
  ...
  ( try
    ...
    with Not_found ->
      ...
      (* Monitor the peer to be sure the notification is closed when
         the peer exits *)
      monitor_peer peer );
  return { result = waiter; notification = notif; peer; id }
