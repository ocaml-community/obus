...
type state = [ `Unknown | `Asleep | `Connecting | `Connected | `Disconnected ]
  ...
  | i ->
      ignore
        (Lwt_log.warning_f ~section
           "Nm_manager.state_of_int32: unknown state: %ld" i);
      `Unknown
...
let proxy daemon =
  OBus_proxy.make daemon [ "org"; "freedesktop"; "NetworkManager" ]

open Nm_interfaces.Org_freedesktop_NetworkManager

let get_devices daemon =
  let%lwt context, devices =
    OBus_method.call_with_context m_GetDevices (proxy daemon) ()
  in
  return
    (List.map
       (fun path ->
         Nm_device.of_proxy (OBus_proxy.make (OBus_context.sender context) path))
       devices)

let activate_connection daemon ~service_name ~connection ~device
    ~specific_object =
  let connection =
    OBus_proxy.path (Nm_settings.Connection.to_proxy connection)
  in
  ...
  let%lwt context, active_connection =
    OBus_method.call_with_context m_ActivateConnection (proxy daemon)
...
let deactivate_connection daemon ~active_connection =
  let active_connection =
    OBus_proxy.path (Nm_connection.to_proxy active_connection)
  in
  OBus_method.call m_DeactivateConnection (proxy daemon) active_connection

let sleep daemon ~sleep = OBus_method.call m_Sleep (proxy daemon) sleep

let wireless_enabled daemon =
  OBus_property.make ~monitor:Nm_monitor.monitor p_WirelessEnabled
    (proxy daemon)

let wireless_hardware_enabled daemon =
  OBus_property.make ~monitor:Nm_monitor.monitor p_WirelessHardwareEnabled
    (proxy daemon)
...
let wwan_hardware_enabled daemon =
  OBus_property.make ~monitor:Nm_monitor.monitor p_WwanHardwareEnabled
    (proxy daemon)
  ...
  OBus_property.map_r_with_context
    (fun context paths ->
      List.map
        ...
        paths)
    (OBus_property.make ~monitor:Nm_monitor.monitor p_ActiveConnections
       (proxy daemon))

let state daemon =
  OBus_property.map_r state_of_int32
    (OBus_property.make ~monitor:Nm_monitor.monitor p_State (proxy daemon))

let state_changed daemon =
  OBus_signal.map state_of_int32
