...
type typ = [ `Unknown | `Ethernet | `Wifi | `Gsm | `Cdma ]

type capability = [ `Nm_supported | `Carrier_detect ]
  ...
  | st ->
      ignore
        (Lwt_log.warning_f ~section
           "Nm_device.state_of_int32: unknown device_state: %ld" st);
      `Unknown
...
let state_reason_of_int32 = function
  ...
  | n ->
      ignore
        (Lwt_log.warning_f ~section
           "Nm_device.state_reason_of_int32: unknown state_reason: %ld" n);
      `Unknown

open Nm_interfaces.Org_freedesktop_NetworkManager_Device

let udi proxy = OBus_property.make ~monitor:Nm_monitor.monitor p_Udi proxy

let interface proxy =
  OBus_property.make ~monitor:Nm_monitor.monitor p_Interface proxy

let driver proxy = OBus_property.make ~monitor:Nm_monitor.monitor p_Driver proxy
...
let device_type proxy =
  OBus_property.map_r
    (function
      ...
      | n ->
          ignore
            (Lwt_log.warning_f ~section "device_type_of_int: unknown type: %ld"
               n);
          `Unknown)
...
let properties proxy =
  OBus_property.group ~monitor:Nm_monitor.monitor proxy
...
module Bluetooth = struct
  ...
  let name proxy = OBus_property.make ~monitor:Nm_monitor.monitor p_Name proxy
...
module Wireless = struct
  ...
  type wifi_mode = [ `Unknown | `Adhoc | `Infra ]

  let get_access_points proxy =
    let%lwt context, access_points =
      OBus_method.call_with_context m_GetAccessPoints proxy ()
    in
    return
      (List.map
         (fun path ->
  ...
  let wireless_capabilities proxy =
    OBus_property.map_r
      (fun x -> Int32.to_int x)
      (OBus_property.make ~monitor:Nm_monitor.monitor p_WirelessCapabilities
         proxy)
