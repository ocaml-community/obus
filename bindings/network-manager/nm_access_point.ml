...
let ap_security_flags_of_int32 n =
  ...
  let add l bit_mask flag = if n land bit_mask <> 0 then flag :: l else l in
  let l = [] in
...
let wpa_flags proxy =
  OBus_property.map_r ap_security_flags_of_int32
...
let ssid proxy = OBus_property.make ~monitor:Nm_monitor.monitor p_Ssid proxy

let frequency proxy =
  OBus_property.map_r Int32.to_int
...
let strength proxy =
  OBus_property.map_r int_of_char
