...
let filesystem_driver_missing =
  "org.freedesktop.UDisks.Error.FilesystemDriverMissing"

let filesystem_tools_missing =
...
let enumerate_adapters daemon =
  let%lwt context, devices =
    OBus_method.call_with_context m_EnumerateAdapters (proxy daemon) ()
  in
  return (List.map (make_adapter context) devices)

let enumerate_expanders daemon =
  let%lwt context, devices =
    OBus_method.call_with_context m_EnumerateExpanders (proxy daemon) ()
  in
  return (List.map (make_expander context) devices)

let enumerate_ports daemon =
  let%lwt context, devices =
    OBus_method.call_with_context m_EnumeratePorts (proxy daemon) ()
  in
  return (List.map (make_port context) devices)

let enumerate_devices daemon =
  let%lwt context, devices =
    OBus_method.call_with_context m_EnumerateDevices (proxy daemon) ()
  in
...
let find_device_by_device_file daemon ~device_file =
  let%lwt context, device =
    OBus_method.call_with_context m_FindDeviceByDeviceFile (proxy daemon)
      device_file
  in
...
let find_device_by_major_minor daemon ~device_major ~device_minor =
  let%lwt context, device =
    OBus_method.call_with_context m_FindDeviceByMajorMinor (proxy daemon)
...
let drive_set_all_spindown_timeouts daemon ~timeout_seconds ~options =
  ...
  OBus_method.call m_DriveSetAllSpindownTimeouts (proxy daemon)
...
let linux_lvm2_vgadd_pv daemon ~uuid ~physical_volume ~options =
  let physical_volume =
    OBus_proxy.path (UDisks_device.to_proxy physical_volume)
  in
  OBus_method.call m_LinuxLvm2VGAddPV (proxy daemon)
    (uuid, physical_volume, options)

let linux_lvm2_vgremove_pv daemon ~vg_uuid ~pv_uuid ~options =
  OBus_method.call m_LinuxLvm2VGRemovePV (proxy daemon)
    (vg_uuid, pv_uuid, options)
...
let linux_lvm2_lvcreate daemon ~group_uuid ~name ~size ~num_stripes ~stripe_size
    ~num_mirrors ~options ~fstype ~fsoptions =
  let num_stripes = Int32.of_int num_stripes in
  let num_mirrors = Int32.of_int num_mirrors in
  let%lwt context, created_device =
    OBus_method.call_with_context m_LinuxLvm2LVCreate (proxy daemon)
      ( group_uuid,
        name,
        size,
        num_stripes,
...
let linux_md_start daemon ~components ~options =
  let components =
    List.map (fun c -> OBus_proxy.path (UDisks_device.to_proxy c)) components
  in
  let%lwt context, device =
    OBus_method.call_with_context m_LinuxMdStart (proxy daemon)
      (components, options)
  in
  return (make_device context device)
...
let linux_md_create daemon ~components ~level ~stripe_size ~name ~options =
  ...
  in
  let%lwt context, device =
    OBus_method.call_with_context m_LinuxMdCreate (proxy daemon)
      (components, level, stripe_size, name, options)
  in
  return (make_device context device)
...
let device_added daemon =
  OBus_signal.map_with_context make_device
...
let adapter_added daemon =
  OBus_signal.map_with_context make_adapter
...
let expander_added daemon =
  OBus_signal.map_with_context make_expander
...
let port_added daemon =
  OBus_signal.map_with_context make_port
