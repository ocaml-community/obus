...
val drive_benchmark :
  t -> do_write_benchmark:bool -> options:string list -> benchmark_result Lwt.t

val drive_ata_smart_initiate_selftest :
  t -> test:string -> options:string list -> unit Lwt.t
...
val drive_unset_spindown_timeout :
  t -> cookie:spindown_timeout_cookie -> unit Lwt.t
...
val drive_inhibit_polling :
  t -> options:string list -> inhibit_polling_cookie Lwt.t
...
val linux_md_remove_component :
  t -> component:t -> options:string list -> unit Lwt.t

val linux_md_expand :
  t -> components:t list -> options:string list -> unit Lwt.t

val linux_md_add_spare : t -> component:t -> options:string list -> unit Lwt.t

val luks_change_passphrase :
  t -> current_passphrase:string -> new_passphrase:string -> unit Lwt.t
...
val filesystem_mount :
  t -> filesystem_type:string -> options:string list -> string Lwt.t
...
val partition_modify :
  t -> typ:string -> label:string -> flags:string list -> unit Lwt.t

val partition_create :
  t ->
  offset:int64 ->
  size:int64 ->
  typ:string ->
...
val partition_table_create :
  t -> scheme:string -> options:string list -> unit Lwt.t
