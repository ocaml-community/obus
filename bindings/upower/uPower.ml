...
let device_added daemon =
  OBus_signal.map_with_context
    (fun context device ->
      UPower_device.of_proxy
        (OBus_proxy.make
           (OBus_context.sender context)
           (OBus_path.of_string device)))
...
let daemon_version daemon =
  OBus_property.make ~monitor:UPower_monitor.monitor p_DaemonVersion
    (proxy daemon)
...
let can_hibernate daemon =
  OBus_property.make ~monitor:UPower_monitor.monitor p_CanHibernate
    (proxy daemon)
...
let on_low_battery daemon =
  OBus_property.make ~monitor:UPower_monitor.monitor p_OnLowBattery
    (proxy daemon)

let lid_is_closed daemon =
  OBus_property.make ~monitor:UPower_monitor.monitor p_LidIsClosed
    (proxy daemon)

let lid_is_present daemon =
  OBus_property.make ~monitor:UPower_monitor.monitor p_LidIsPresent
    (proxy daemon)
