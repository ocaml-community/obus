...
let _ =
  ...
  let hello_message =
    method_call (* function for creating methoc_call header *)
  ...
  let (_, body) = reply in
    ...
    (* [body] is composed of a sequence of dbus values, in a "marshaled"
       representation.  We now use [get_values] to unmarshal the
       values into a simple ocaml value. If the body does not contain
       what we are claiming (here it is a sequence of just one
       string), this is an error, and [get_values] will raise an
       exception *)
    try
      let (name, ()) = get_values (ccons cstring cnil) body in
