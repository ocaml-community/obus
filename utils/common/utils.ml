...
let parse_xml fname =
  ...
  try
    let interfaces, _ =
      OBus_introspect.input
        (Xmlm.make_input ~entity:(fun _ -> Some "") ~strip:true (`Channel ic))
    in
    close_in ic;
...
let file_name_of_interface_name name =
  ...
  for i = 0 to String.length name - 1 do
    if name.[i] = '.' then Bytes.set result i '_'
    else Bytes.set result i name.[i]
  done;
...
let rec convertor conv top = function
  ...
  | Term ("dict", [ tk; tv ]) -> (
      ...
      | Some fk, Some fv ->
          Some
            (paren top (sprintf "List.map (fun (k, v) -> (%s k, %s v))" fk fv))
      )
  | Term (name, args) -> None
  | Tuple tl ->
      let l = List.map (convertor conv true) tl in
      if List.exists (fun f -> f <> None) l then
        let names = make_names tl in
        Some
          (sprintf "(fun (%s) -> (%s))" (String.concat ", " names)
             (String.concat ", "
                (List.map2
                   (fun name conv ->
                     match conv with
...
let convertor_recv top typ =
  convertor
    (fun top t ->
      ...
      | "object_path" ->
          Some (paren top "OBus_proxy.make (OBus_context.sender context)")
      | name when List.mem name dbus_symbols -> None
...
let make_annotation = function
  ...
  | "org.freedesktop.DBus.Property.EmitsChangedSignal" ->
      "OBus_introspect.emits_changed_signal"
