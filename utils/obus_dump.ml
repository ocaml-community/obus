  ...
  let%lwt server =
    OBus_server.make_lowlevel ~capabilities:[ `Unix_fd ]
      (fun server transport ->
        ignore
          (let%lwt _, bus =
             OBus_transport.of_addresses ~capabilities:[ `Unix_fd ] addresses
           in
           Lwt.choose
             [
               loop pp "message received" what_bus bus transport;
               loop pp "sending message" what_bus transport bus;
             ]))
  in
  Unix.putenv
    (Printf.sprintf "DBUS_%s_BUS_ADDRESS" (String.uppercase_ascii what_bus))
    (OBus_address.to_string (OBus_server.addresses server));
  Lwt.return ()
...
let () =
  ...
  Lwt_main.run
    (let%lwt () =
       launch pp "session" OBus_address.session
       <&> launch pp "system" OBus_address.system
     in
     let%lwt _ =
       Lwt_unix.waitpid []
         (Unix.create_process cmd (Array.of_list cmd_args) Unix.stdin
