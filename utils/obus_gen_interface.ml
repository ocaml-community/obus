...
let term_intf typ = Term.intf (OBus_introspect_ext.term_of_single typ)

let term_impl typ = Term.impl (OBus_introspect_ext.term_of_single typ)

let tuple_intf types = Term.intf (OBus_introspect_ext.term_of_sequence types)

let tuple_impl types = Term.impl (OBus_introspect_ext.term_of_sequence types)
    ...
    (function
      | Method (name, i_args, o_args, annotations) ->
          fprintf oc "    m_%s : 'a OBus_object.t -> %a -> %a;\n" name
            ...
            (Term.print_intf true)
            (term "Lwt.t"
               [ tuple (List.map (fun (name, typ) -> term_intf typ) o_args) ])
      ...
      | Property (name, typ, Write, annotations) ->
          fprintf oc "    p_%s : 'a OBus_object.t -> %a -> unit Lwt.t;\n" name
            (Term.print_intf true) (term_intf typ)
      | Property (name, typ, Read_write, annotations) ->
          fprintf oc
            "    p_%s : ('a OBus_object.t -> %a) * ('a OBus_object.t -> %a -> \
             unit Lwt.t);\n"
            name (Term.print_intf true)
            (term "React.signal" [ term_intf typ ])
            (Term.print_intf true) (term_intf typ))
    members;
  output_string oc "  }\n"
...
let print_symbol oc name sym =
  ...
  | (key, name) :: rest ->
      ...
      List.iter
        (fun (key, name) ->
          fprintf oc "\n    | `%s" (String.capitalize_ascii name))
        rest;
      fprintf oc " ]\n"
...
let string_of_integer_enum = function
  | V.Byte x -> sprintf "%C" x
  | V.Int16 x | V.Uint16 x -> sprintf "%d" x
  | V.Int32 x | V.Uint32 x -> sprintf "%ldl" x
  | V.Int64 x | V.Uint64 x -> sprintf "%LdL" x
  | _ -> assert false

let string_of_integer_flag = function
  | V.Byte x -> sprintf "%d" (Char.code x)
  | V.Int16 x | V.Uint16 x -> sprintf "%d" x
  | V.Int32 x | V.Uint32 x -> sprintf "%ldl" x
  | V.Int64 x | V.Uint64 x -> sprintf "%LdL" x
  | _ -> assert false
...
let print_impl oc name members symbols annotations =
    ...
    (fun (name, sym) ->
      ...
      | Sym_enum (typ, values) ->
          ...
          List.iter
            ...
            values;
          fprintf oc
            "    | n -> Printf.ksprintf failwith \"invalid value for \
             \\\"%s\\\": %s\" n\n"
            name
            ( match typ with
            | T.Byte -> "%c"
      ...
      | Sym_flag (typ, values) ->
          ...
          List.iter
            (fun (key, name) ->
              fprintf oc "      | `%s :: rest -> loop (%s) rest\n"
                (String.capitalize_ascii name)
                ( match key with
                | V.Byte x -> sprintf "acc lor %d" (Char.code x)
                | V.Int16 x | V.Uint16 x -> sprintf "acc lor %d" x
                | V.Int32 x | V.Uint32 x -> sprintf "Int32.logor acc %ldl" x
                | V.Int64 x | V.Uint64 x -> sprintf "Int64.logor acc %LdL" x
                | _ -> assert false ))
            values;
          fprintf oc "    in\n    loop %s l\n"
            ( match typ with
          ...
          if typ = T.Byte then fprintf oc "    let n = int_of_char n in\n";
          List.iter
            (fun (key, name) ->
              fprintf oc "    let l = if %s then `%s :: l else l in\n"
                ( match key with
                | V.Byte x -> sprintf "n land %d <> 0" (Char.code x)
                | V.Int16 x | V.Uint16 x -> sprintf "n land %d <> 0" x
      ...
      | Method (name, i_args, o_args, annotations) ->
          fprintf oc
            ...
            (String.concat ";\n                          "
               (List.map
                  (fun (name, value) ->
                    sprintf "(%s, %S)" (Utils.make_annotation name) value)
                  annotations))
      | Signal (name, args, annotations) ->
          fprintf oc
            "  let s_%s = {\n\
  ...
  if !mode <> `Client then (
    if
      List.exists
        (function Method _ | Property _ -> true | _ -> false)
        members
    then print_record oc members;
    ...
    List.iter
      (function
        ...
        | Property (name, typ, Read_write, annotations) ->
            fprintf oc
              "        property_rw_info p_%s (fst members.p_%s) (snd \
               members.p_%s);\n"
              name name name
        | _ -> ())
      members;
...
let print_intf oc name members symbols annotations =
    ...
    (function
      ...
      | Signal (name, args, annotations) ->
          fprintf oc "  val s_%s : %a\n" name (Term.print_intf true)
            (term "Signal.t" [ tuple_intf (List.map snd args) ])
      | Property (name, typ, access, annotations) ->
  ...
  if !mode <> `Client then
    ...
    else output_string oc "  val make : unit -> 'a OBus_object.interface\n";
  output_string oc "end\n"
...
let compare_members ma mb =
  ...
  | Signal (name_a, _, _), Signal (name_b, _, _) -> String.compare name_a name_b
...
let args =
  [
    ...
    ( "-o",
      Arg.String (fun str -> prefix := Some str),
...
let () =
  ...
  let prefix =
    ...
    | Some str -> str
    | None ->
        (try Filename.chop_extension source with Invalid_argument _ -> source)
        ^ "_interfaces"
  in
  let oc_impl = open_out (prefix ^ ".ml")
  and oc_intf = open_out (prefix ^ ".mli") in
  fprintf oc_impl
    "(* File auto-generated by %s, DO NOT EDIT. *)\n\
     ...
     open OBus_member\n"
    prog_name;
  if !mode <> `Client then output_string oc_impl "open OBus_object\n";
  fprintf oc_intf
    "(* File auto-generated by %s, DO NOT EDIT. *)\nopen OBus_member\n"
    prog_name;
  Utils.IFSet.iter
    (fun ((name, members, symbols, annotations) as interface) ->
      ...
      then (
        ...
        let members = sort_members members
        and annotations = List.sort compare annotations in
