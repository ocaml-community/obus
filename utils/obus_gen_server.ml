  ...
  List.iter
    (function
      | Method (name, i_args, o_args, annotations) ->
          ...
          and o_convertors =
            make_convertors Utils.convertor_send o_names o_args
          in
          fprintf oc "      m_%s = (\n        fun obj %a ->\n" name print_names
            i_names;
          List.iter
            (function
              ...
              | None -> ())
            i_convertors;
          fprintf oc "          let%%lwt %a = %s (OBus_object.get obj)"
            print_names o_names (OBus_name.ocaml_lid name);
  ...
  Utils.IFSet.iter
    (fun (name, members, symbols, annotations) ->
      if
        ...
        | _ -> true
      then print_impl oc name members symbols annotations)
    interfaces;
  close_out oc;
