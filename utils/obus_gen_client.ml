  ...
  List.filter
    (function
      ...
      | Property (_, _, _, annotations) -> (
          try List.assoc OBus_introspect.deprecated annotations <> "true"
          with Not_found -> true ))
    members
...
let make_convertors make_convertor names args =
  List.map2
    (fun (_, name) (_, typ) ->
      match make_convertor true typ with
...
let print_impl oc name members symbols annotations =
  ...
  List.iter
    (fun (name, sym) -> fprintf oc "  type %s = type_%s\n" name name)
    symbols;
  List.iter
    (function
      | Method (name, i_args, o_args, annotations) ->
          ...
          and o_convertors =
            make_convertors Utils.convertor_recv o_names o_args
          in
          ...
          output_string oc " =\n";
          List.iter
            (function Some line -> fprintf oc "    %s" line | None -> ())
            i_convertors;
          let need_context =
            List.exists (fun (_, typ) -> contains_path typ) o_args
          in
          if List.for_all (fun conv -> conv = None) o_convertors then (
            if
              try List.assoc OBus_introspect.no_reply annotations = "true"
              with Not_found -> false
            then fprintf oc "    OBus_method.call_no_reply m_%s proxy " name
            else fprintf oc "    OBus_method.call m_%s proxy " name;
      ...
      | Signal (name, args, annotations) ->
          ...
          if List.for_all (fun x -> x = None) convertors then
            ...
            else output_string oc "    OBus_signal.map\n      (fun ";
            print_names oc names;
            output_string oc " ->\n";
            List.iter
              (function
                | Some line -> fprintf oc "         %s" line | None -> ())
              convertors;
            output_string oc "         ";
            print_names oc names;
            output_string oc ")\n";
      ...
      | Property (name, typ, access, annotations) -> (
          fprintf oc "\n  let %s proxy =\n" (OBus_name.ocaml_lid name);
          match
            (Utils.convertor_recv true typ, Utils.convertor_send true typ)
          with
          | Some f_recv, Some f_send ->
          ...
          | None, None -> fprintf oc "    OBus_property.make p_%s proxy\n" name
          | _ -> assert false ))
    members;
...
let rec term_intf = function
  ...
  | Term ("dict", [ tk; tv ]) ->
      term "list" [ tuple [ term_intf tk; term_intf tv ] ]
      ...
      | Method (name, i_args, o_args, annotations) ->
          ...
          List.iter
            (function
              ...
              | Some name, typ ->
                  fprintf oc "%s : %a -> " name (Term.print_intf true)
                    (term_intf typ))
            i_args;
          Term.print_intf true oc
            (term "Lwt.t"
               [ tuple (List.map (fun (_, typ) -> term_intf typ) o_args) ]);
          output_char oc '\n'
      | Signal (name, args, annotations) ->
          fprintf oc "  val %s : OBus_proxy.t -> %a\n"
            (OBus_name.ocaml_lid name) (Term.print_intf true)
            (term "OBus_signal.t"
