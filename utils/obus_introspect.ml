...
let args =
  [
    ...
    ( "-address",
      Arg.String (fun addr -> address := Some addr),
      "the service is on the given message bus" );
    ( "-objects",
      Arg.Set obj_mode,
...
let usage_msg =
  Printf.sprintf
    "Usage: %s <option> <destination> <path>\n\
...
let rec get proxy =
  ...
  let map =
    List.fold_left
      (fun map (name, content, annots) ->
        Interface_map.add name (content, annots) map)
      Interface_map.empty interfaces
  in
  ...
  | true ->
      List.fold_left
        (fun t1 t2 ->
          ...
          Lwt.return
            (nodes1 @ nodes2, Interface_map.fold Interface_map.add map1 map2))
        ...
        (List.map
           (fun child ->
             get
               {
                 proxy with
                 OBus_proxy.path = OBus_proxy.path proxy @ [ child ];
               })
           children)
  | false -> Lwt.return (nodes, map)

let main service path =
  let%lwt bus =
    match (!session, !system, !address) with
    | true, true, _ | true, _, Some _ | _, true, Some _ ->
        prerr_endline "must specify at most one of -session, -system\n\n";
        Arg.usage args usage_msg;
        exit 1
    | false, false, None | true, false, None -> OBus_bus.session ()
    | false, true, None -> OBus_bus.system ()
    | false, false, Some addr ->
        OBus_bus.of_addresses (OBus_address.of_string addr)
  in
  let%lwt nodes, map =
    get (OBus_proxy.make (OBus_peer.make bus service) path)
  in
  ( match !obj_mode with
  | false ->
      OBus_introspect.output
        (Xmlm.make_output ~nl:true ~indent:(Some 2)
           (`Channel Pervasives.stdout))
        ( Interface_map.fold
            (fun name (content, annots) acc -> (name, content, annots) :: acc)
            map [],
          [] )
  | true ->
      List.iter
        (fun (proxy, interfaces) ->
...
let () =
  Arg.parse args (fun arg -> anons := arg :: !anons) usage_msg;
  let service, path =
    match !anons with
    ...
    | _ ->
        ...
        exit 1
  in
  try Lwt_main.run (main service path)
  with OBus_introspect.Parse_failure ((line, column), msg) ->
    Lwt.ignore_result
      (Lwt_io.eprintlf
         "invalid introspection document returned by the service!:%d:%d: %s"
         line column msg);
    exit 1
