...
let () =
  Lwt_main.run
    (let%lwt bus = OBus_bus.session () in
     ...
     let%lwt () =
       printlf "the message bus id is: %S" (OBus_uuid.to_string id)
     in
     ...
     let%lwt result =
       OBus_bus.request_name bus ~replace_existing:true ~do_not_queue:true name
     in
     let%lwt () =
       printl
         ( match result with
     ...
     in
     printlf "my names are: %s"
       (String.concat ", " (String_set.elements (S.value (OBus_bus.names bus)))))
