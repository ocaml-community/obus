...
let handle_multimedia_keys device =
  ...
  >|= E.map_p (fun (action, key) ->
          let%lwt () = printlf "from Hal: action %S on key %S!" action key in
          let%lwt () =
            printlf "          the signal come from the device %S"
              (OBus_path.to_string (Hal_device.udi device))
          in
          return ())
  >|= E.keep
  ...
  Lwt_main.run
    (let%lwt session = OBus_bus.session () in
     ...
     let%lwt () =
       ...
       >|= E.map_p (fun (name, old_owner, new_owner) ->
     ...
     let%lwt () =
       ...
       >|= E.map_p (fun device ->
               let%lwt () =
                 printlf "from Hal: device added: %S"
                   (OBus_path.to_string (Hal_device.udi device))
               in
     ...
     let%lwt keyboards =
       Hal_manager.find_device_by_capability manager "input.keyboard"
     in
     let%lwt () = printlf "keyboard founds: %d" (List.length keyboards) in
     let%lwt () =
       Lwt_list.iter_p
         (fun dev -> printlf "  %s" (OBus_path.to_string (Hal_device.udi dev)))
         keyboards
     in
