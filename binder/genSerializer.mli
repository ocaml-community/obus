...
val rule_set : string -> poly caml_type -> rule
  (** [rule_set module_name elt_type] shorthand for array created with
      [Set.Make] *)

val rule_map : string -> poly caml_type -> rule
...
val rule_variant : poly caml_type -> poly caml_type -> (patt * expr * string * poly caml_type list * dtypes) list -> rule
  (** [rule_variant typ key_type variants] rule for reading/writing a
      dbus marshaled variant into/from this caml variant *)

val rule_record_option : poly caml_type -> poly caml_type -> (patt * expr * string * poly caml_type * dtypes) list -> rule
...
val gen_reader : rule list -> mono caml_type -> mono dbus_type -> env -> env * code
  (** [gen_reader rules caml_type dbus_type env] generate a function
      that read a dbus serialized value of type [dbus_type] into a
      caml value of type [caml_type].

      It can generate auxiliary functions which are added to [env]. *)

val gen_writer : rule list -> mono caml_type -> mono dbus_type -> env -> env * code
  (** [gen_writer rules dbus_type caml_type env] like [gen_reader] but
      for writing *)
