...
let () =
  Printexc.register_printer (function
    | Connection_closed -> Some "D-Bus connection closed"
    | Connection_lost -> Some "D-Bus connection lost"
    | Transport_error exn ->
        Some
          (Printf.sprintf "D-Bus transport failure: %s"
...
module Serial_map = Map.Make (struct
...
type active_connection = {
  ...
  wrapper : t; (* The wrapper containing the connection *)
}
...
let apply_filters typ message filters =
  try
    Lwt_sequence.fold_l
      (fun filter message ->
        match message with Some message -> filter message | None -> None)
...
let cleanup active ~is_crash =
  ...
  | Some (waiter, wakener) -> Lwt.wakeup_exn wakener Connection_closed
  | None -> () );
  (* Wakeup all reply handlers so they will not wait forever *)
  Serial_map.iter
    (fun _ wakener -> Lwt.wakeup_exn wakener Connection_closed)
    active.reply_waiters;
  (* If the connection is closed normally, flush it *)
  let%lwt () =
    if not is_crash then Lwt_mutex.with_lock active.outgoing_mutex Lwt.return
    else (
      Lwt.wakeup_exn active.abort_send_wakener Connection_closed;
      Lwt.return () )
  in
  (* Shutdown the transport *)
  try%lwt OBus_transport.shutdown active.transport
  with exn ->
    Lwt_log.error ~section ~exn "failed to abort/shutdown the transport"

let close connection =
  match connection#state with
  | Killed | Closed -> Lwt.return ()
  | Active active ->
...
let kill connection exn =
  ...
  | Active active -> (
      ...
      try%lwt active.on_disconnect exn
      with exn -> Lwt_log.error ~section ~exn "the error handler failed with" )
...
let send_message_backend connection gen_serial reply_waiter_opt message =
  ...
  Lwt_mutex.with_lock active.outgoing_mutex (fun () ->
      let send_it, closed =
        match connection#state with
        | Active _ -> (true, false)
        | Closed ->
      ...
      if send_it then
        let message =
          if gen_serial then
            { message with OBus_message.serial = active.next_serial }
          else message
        in
        match apply_filters "outgoing" message active.outgoing_filters with
        | None ->
            let%lwt () =
              Lwt_log.debug ~section "outgoing message dropped by filters"
            in
        ...
        | Some message -> (
            ( if not closed then
              ...
              | Some (waiter, wakener) ->
                  active.reply_waiters <-
                    Serial_map.add
                      (OBus_message.serial message)
                      wakener active.reply_waiters;
            ...
            try%lwt
              let%lwt () =
                Lwt.choose
                  [
                    active.abort_send_waiter;
                    (* Do not cancel a thread while it is marshaling message: *)
                    Lwt.protected (OBus_transport.send active.transport message);
                  ]
              in
        ...
        match connection#state with
        | Killed | Closed -> Lwt.fail Connection_closed
        | Active _ -> Lwt.return ())
...
let method_call_with_message ~connection ?destination ~path ?interface ~member
    ~i_args ~o_args args =
  let i_msg =
    OBus_message.method_call ?destination ~path ?interface ~member
      (OBus_value.C.make_sequence i_args args)
  in
  let%lwt o_msg = send_message_with_reply connection i_msg in
  match o_msg with
  | { OBus_message.typ = OBus_message.Method_return _; body } -> (
      try Lwt.return (o_msg, OBus_value.C.cast_sequence o_args body)
      with OBus_value.C.Signature_mismatch ->
        Lwt.fail
          (OBus_message.invalid_reply i_msg
             (OBus_value.C.type_sequence o_args)
             o_msg) )
  | {
...
let method_call ~connection ?destination ~path ?interface ~member ~i_args
    ~o_args args =
  method_call_with_message ~connection ?destination ~path ?interface ~member
    ~i_args ~o_args args
  >|= snd

let method_call_no_reply ~connection ?destination ~path ?interface ~member
    ~i_args args =
  send_message connection
    (OBus_message.method_call
       ~flags:
         {
           OBus_message.default_flags with
           OBus_message.no_reply_expected = true;
         }
...
let dispatch_message active message =
  ...
  (* For method return and errors, we lookup at the reply waiters. If
     ...
     dropped. *)
  | { typ = Method_return reply_serial } | { typ = Error (reply_serial, _) }
    -> (
      match
        try Some (Serial_map.find reply_serial active.reply_waiters)
        with Not_found -> None
      with
      | Some w ->
          active.reply_waiters <-
            Serial_map.remove reply_serial active.reply_waiters;
      ...
      | None ->
          Lwt_log.debug_f ~section "reply to message with serial %ld dropped%s"
            ...
            | { typ = Error (_, error_name) } ->
                Printf.sprintf ", the reply is the error: %S: %S" error_name
  ...
  | {
   typ = Method_call (_, "org.freedesktop.DBus.Peer", member);
   body;
   sender;
   serial;
  } -> (
        ...
        let%lwt body =
          ...
          | "GetMachineId", [] -> (
              try%lwt
                ...
                Lwt.return
                  [ OBus_value.V.basic_string (OBus_uuid.to_string uuid) ]
              with exn ->
                if OBus_error.name exn = OBus_error.ocaml then
                  Lwt.fail
                    (OBus_error.Failed
                       (Printf.sprintf "Cannot read the machine uuid file (%s)"
          ...
          | _ ->
              Lwt.fail
                (OBus_error.Unknown_method
                   (Printf.sprintf
                      "Method %S with signature %S on interface \
                       \"org.freedesktop.DBus.Peer\" does not exist"
                      member
...
let rec dispatch_forever active =
  let%lwt () =
    ...
    match S.value active.down with
    | Some (waiter, wakener) -> waiter
    | None -> Lwt.return ()
  in
  let%lwt message =
    try%lwt
      Lwt.choose
        [ OBus_transport.recv active.transport; active.abort_recv_waiter ]
    with exn ->
  ...
  | None ->
      let%lwt () =
        Lwt_log.debug ~section "incoming message dropped by filters"
      in
      dispatch_forever active
  | Some message ->
      (* The internal dispatcher accepts only messages destined to
         the current connection: *)
      if active.name = "" || OBus_message.destination message = active.name then
        ignore
          (( try%lwt dispatch_message active message
             with exn ->
...
class connection () =
  ...
  object (self)
    ...
    method set_state new_state =
      state <- new_state;
      match state with
      | Closed | Killed -> set_active false
      | Active _ -> set_active true
...
let of_transport ?switch ?guid ?(up = true) transport =
  ...
  | Some guid -> (
      match
        try Some (Guid_map.find guid !guid_connection_map)
        with Not_found -> None
      with
      | Some connection ->
          Lwt_switch.add_hook switch (fun () -> close connection);
      ...
      | None ->
          ...
          guid_connection_map :=
            Guid_map.add guid connection !guid_connection_map;
          connection )
...
let of_addresses ?switch ?(shared = true) addresses =
  ...
  | false ->
      let%lwt guid, transport =
        OBus_transport.of_addresses ~capabilities addresses
      in
...
type 'a key = { key_id : int; key_make : 'a -> exn; key_cast : exn -> 'a }
...
let set connection key value =
  let active = connection#get in
  match value with
  | Some x -> active.data <- Int_map.add key.key_id (key.key_make x) active.data
  | None -> active.data <- Int_map.remove key.key_id active.data
...
let rec can_send_single_type connection = function
  ...
  | OBus_value.T.Dict (tk, tv) ->
      can_send_basic_type connection tk && can_send_single_type connection tv
  | OBus_value.T.Structure tl ->
      List.for_all (can_send_single_type connection) tl
  | OBus_value.T.Variant -> true

let can_send_sequence_type connection tl =
  List.for_all (can_send_single_type connection) tl

let set_on_disconnect connection f =
  match connection#state with
  | Closed | Killed -> ()
  | Active active -> active.on_disconnect <- f
...
let set_down connection =
  ...
  match S.value active.down with
  | Some _ -> ()
  | None -> active.set_down (Some (Lwt.wait ()))
