...
let read_nonce fd =
  ...
  let rec loop ofs len =
    ...
    | n ->
        if n = len then Lwt.return (Bytes.unsafe_to_string nonce)
        else loop (ofs + n) (len - n)
  in
  loop 0 16
  ...
  ( try%lwt
    ...
    with Unix_error (err, _, _) ->
      let%lwt () =
        ...
        else (* Ignore errors that happens after a shutdown *)
          Lwt.return ()
      in
      Lwt.return `Shutdown )
  >>= function
  | `Accept (fd, address) ->
      if OBus_address.name listener.lst_address = "nonce-tcp" then
        ( try%lwt
            let%lwt nonce = read_nonce fd in
            if nonce <> server.srv_nonce then
              let%lwt () =
                Lwt_log.notice_f ~section
                  "client rejected because of invalid nonce"
              in
              Lwt.return `Drop
            else Lwt.return `OK
          with
        | End_of_file ->
            let%lwt () =
              Lwt_log.warning ~section "cannot read nonce from socket"
            in
            Lwt.return `Drop
        | Unix.Unix_error (err, _, _) ->
            let%lwt () =
              Lwt_log.warning_f ~section "cannot read nonce from socket: %s"
                (Unix.error_message err)
            in
            Lwt.return `Drop )
        >>= function
        | `OK -> Lwt.return (Event_connection (fd, address))
        | `Drop ->
            let%lwt () =
              try
      ...
      else Lwt.return (Event_connection (fd, address))
  | `Shutdown -> Lwt.return Event_shutdown
...
let cleanup address =
  ...
  | "unix" -> (
      ...
      | Some path -> (
          ...
          with Unix_error (err, _, _) ->
            Lwt_log.error_f ~section "cannot unlink '%s': %s" path
              (Unix.error_message err) )
      | None -> Lwt.return () )
  | _ -> Lwt.return ()
...
let string_of_address = function
  | ADDR_UNIX path ->
      ...
      else Printf.sprintf "unix path %S" path
  | ADDR_INET (ia, port) ->
...
let handle_client server listener fd address =
  ...
  try%lwt
    ...
    | 0 ->
        Lwt.fail
          (OBus_auth.Auth_failure "did not receive the initial null byte")
    | 1 -> (
        let user_id =
          try Some (Lwt_unix.get_credentials fd).Lwt_unix.cred_uid
          with Unix.Unix_error (error, _, _) ->
            ignore
              (Lwt_log.info_f ~section "cannot read credential: %s"
                 (Unix.error_message error));
            None
        in
        let%lwt user_id, capabilities =
          OBus_auth.Server.authenticate ~capabilities:listener.lst_capabilities
            ?mechanisms:server.srv_mechanisms ?user_id ~guid:listener.lst_guid
            ~stream:(OBus_auth.stream_of_fd fd)
            ()
        in
        if user_id = None && not server.srv_allow_anonymous then
          let%lwt () =
            Lwt_log.notice_f ~section
              "client from %s rejected because anonymous connections are not \
               allowed"
...
let rec lst_loop server listener =
  ...
  | Event_shutdown ->
      let%lwt () =
        try Lwt_unix.close listener.lst_fd
        with Unix_error (err, _, _) ->
          Lwt_log.error_f ~section "cannot close listenning socket: %s"
            (Unix.error_message err)
      in
...
let make_socket domain typ address =
  ...
  with Unix_error (err, _, _) as exn ->
    let%lwt () =
      Lwt_log.error_f ~section "failed to create listenning socket with %s: %s"
        (string_of_address address)
        (Unix.error_message err)
    in
...
let fd_addr_list_of_address address =
  ...
  | "unix" -> (
      match
        ( OBus_address.arg "path" address,
          OBus_address.arg "abstract" address,
          OBus_address.arg "tmpdir" address )
      with
      | Some path, None, None ->
      ...
      | None, None, Some tmpd -> (
          ...
          try%lwt
            ...
            Lwt.return
              [
                (fd, OBus_address.make ~name:"unix" ~args:[ ("abstract", path) ]);
              ]
          with exn ->
            (* And fallback to path in the filesystem *)
            let%lwt fd = make_path path in
            Lwt.return
              [ (fd, OBus_address.make ~name:"unix" ~args:[ ("path", path) ]) ]
          )
      | _ ->
          Lwt.fail
            (Invalid_argument
               "OBus_transport.connect: invalid unix address, must supply \
                exactly one of 'path', 'abstract', 'tmpdir'") )
  | ("tcp" | "nonce-tcp") as name -> (
      let port =
        match OBus_address.arg "port" address with
        | Some port -> port
        | None -> "0"
      and bind =
        match OBus_address.arg "bind" address with
        | Some bind -> bind
        | None -> (
            match OBus_address.arg "host" address with
            | Some host -> host
            | None -> "*" )
      in
      ...
      let opts =
        ...
        | Some family ->
            Printf.ksprintf invalid_arg
              "OBus_server.make_server: unknown address family '%s'" family
        | None -> opts
      in
      let ais = getaddrinfo bind port opts in
      (* Remove duplicate address info: *)
      let module AI_set = Set.Make (struct
        type t = addr_info
      ...
      let ais =
        AI_set.elements
          (List.fold_left (fun set ai -> AI_set.add ai set) AI_set.empty ais)
      in
      match ais with
      | [] ->
      ...
      | ais ->
            ...
            Lwt_list.map_p
              (fun ai ->
                try%lwt
                  ...
                  | ADDR_UNIX path -> assert false
                  | ADDR_INET (host, port) ->
                      Lwt.return
                        (`Success
                          ( fd,
                            OBus_address.make ~name
                              ~args:
                                [
                                  ("host", string_of_inet_addr host);
          ...
          if fd_addr_list = [] then
            ...
            match
              OBus_util.find_map
                (function `Failure e -> Some e | `Success _ -> None)
                results
            with
            | Some exn -> Lwt.fail exn
            | None -> assert false
          else Lwt.return fd_addr_list )
  | "autolaunch" ->
      Lwt.fail
        (Failure
           "OBus_server.make_server: autolaunch can not be used as a \
            listenning address")
  | name ->
      Lwt.fail
        (Failure ("OBus_server.make_server: unknown transport type: " ^ name))
  ...
  if server.srv_up then (
    ...
    let%lwt () =
      if server.srv_nonce_file <> "" then
        try Lwt_unix.unlink server.srv_nonce_file
        with Unix_error (err, _, _) ->
    ...
    server.srv_loops )
  else server.srv_loops

let default_address =
  OBus_address.make ~name:"unix"
    ~args:[ ("tmpdir", Filename.get_temp_dir_name ()) ]

let make_lowlevel ?switch ?(capabilities = OBus_auth.capabilities) ?mechanisms
    ?(addresses = [ default_address ]) ?(allow_anonymous = false) callback =
  ...
  | addresses -> (
                    ...
                    (fun (fd, address) ->
                      ...
                      with Unix_error (err, _, _) ->
                        Lwt_log.error_f ~section
                          "failed to close listenning file descriptor: %s"
                          (Unix.error_message err))
                    fd_addr_list
              | `Failure e -> Lwt.return ())
            result_by_address
        in
        Lwt.fail exn
      in
      ...
      | None ->
              ...
              try%lwt
                let%lwt () =
                  Lwt_io.with_file ~mode:Lwt_io.output file_name (fun oc ->
                      Lwt_io.write oc nonce)
                in
                Lwt.return (nonce, file_name)
              with Unix.Unix_error (err, _, _) ->
                abort
                  (Failure
                     (Printf.sprintf "cannot create nonce file '%s': %s"
            ...
            List.map2
              (fun fd_addr_list guid ->
                List.map
                  (fun (fd, addr) ->
                    let args =
                      ("guid", OBus_uuid.to_string guid)
                      :: OBus_address.args addr
                    in
                    let args =
          ...
          server.srv_loops <-
            Lwt.join
              (List.map (fun listener -> lst_loop server listener) listeners);
          let%lwt () =
