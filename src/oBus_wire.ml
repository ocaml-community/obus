...
let () =
  Printexc.register_printer (function
    | Data_error msg -> Some (sprintf "failed to marshal D-Bus message: %s" msg)
    | Protocol_error msg -> Some (sprintf "D-Bus protocol error: %s" msg)
    | _ -> None)
...
let signature_too_long s len =
  sprintf "too long signature: '%s', with len %d" (string_of_signature s) len

let invalid_protocol_version ver =
  sprintf "invalid protocol version: %d (obus implement protocol version %d)"
    ver OBus_info.protocol_version
...
let pad8_p = function
  | T.Structure _ | T.Basic T.Int64 | T.Basic T.Uint64 | T.Basic T.Double ->
      true
  | _ -> false
...
let missing_field message_type_name field_name =
  raise
    (Protocol_error
       (sprintf "invalid header, field '%s' is required for '%s'" field_name
          message_type_name))
...
let get_required_option message_type_name field_name = function
  | None -> missing_field message_type_name field_name
  | Some value -> value

let method_call_of_raw fields =
  Method_call
    ( get_required_option "method-call" "path" fields.rf_path,
      fields.rf_interface,
      get_required_string "method-call" "member" fields.rf_member )

let method_return_of_raw fields =
  Method_return
    (get_required_option "method-return" "reply-serial" fields.rf_reply_serial)

let error_of_raw fields =
  Error
    ( get_required_option "error" "reply-serial" fields.rf_reply_serial,
      get_required_string "error" "error-name" fields.rf_error_name )

let signal_of_raw fields =
  Signal
    ( get_required_option "signal" "path" fields.rf_path,
...
let map_exn f = function
  | OBus_string.Invalid_string err -> raise (f (OBus_string.error_message err))
...
module Count = struct
  ...
  type counter = {
    mutable ofs : int;
    (* Simulate an offset *)
    mutable fds : FD_set.t; (* Set used to collect all file descriptors *)
  }
  ...
  let rec tsingle c = function
    ...
    | T.Structure l ->
        c.ofs <- c.ofs + 2;
        iter tsingle c l
    | T.Variant -> c.ofs <- c.ofs + 1

  let tsequence c l = iter tsingle c l

  let rec tsingle_of_single c = function
    | V.Basic x -> c.ofs <- c.ofs + 1
    | V.Array (t, x) ->
        c.ofs <- c.ofs + 1;
        tsingle c t
    | V.Byte_array _ -> c.ofs <- c.ofs + 2
    | V.Dict (tk, tv, x) ->
        c.ofs <- c.ofs + 4;
        tsingle c tv
    | V.Structure l ->
    ...
    | V.Variant x -> c.ofs <- c.ofs + 1

  let tsequence_of_sequence c l = iter tsingle_of_single c l

  let rec basic c = function
    | V.Byte _ -> c.ofs <- c.ofs + 1
    | V.Int16 _ | V.Uint16 _ -> c.ofs <- pad2 c.ofs + 2
    | V.Boolean _ | V.Int32 _ | V.Uint32 _ -> c.ofs <- pad4 c.ofs + 4
    | V.Int64 _ | V.Uint64 _ | V.Double _ -> c.ofs <- pad8 c.ofs + 8
    | V.String s -> c.ofs <- pad4 c.ofs + String.length s + 5
    | V.Signature s ->
        c.ofs <- c.ofs + 2;
        tsequence c s
    | V.Object_path p -> c.ofs <- pad4 c.ofs + path_length p + 5
    | V.Unix_fd fd ->
        c.ofs <- pad4 c.ofs + 4;
  ...
  let rec single c = function
    ...
    | V.Array (t, l) ->
        ...
        iter single c l
    | V.Byte_array bytes -> c.ofs <- pad4 c.ofs + 4 + String.length bytes
    | V.Dict (tk, tv, l) ->
  ...
  let sequence c l = iter single c l
  ...
  let message msg =
    ...
    | Method_call (path, interface, member) | Signal (path, interface, member)
      ->
        c.ofs <- pad8 c.ofs + 9 + path_length path;
        c.ofs <- pad8 c.ofs + 9 + String.length interface;
        c.ofs <- pad8 c.ofs + 9 + String.length member
    | Method_return serial -> c.ofs <- pad8 c.ofs + 8
    | Error (serial, name) ->
        c.ofs <- pad8 c.ofs + 9 + String.length name;
        c.ofs <- pad8 c.ofs + 8 );
    if msg.destination <> "" then
      c.ofs <- pad8 c.ofs + 9 + String.length msg.destination;
    if msg.sender <> "" then c.ofs <- pad8 c.ofs + 9 + String.length msg.sender;
    (* The signature *)
    c.ofs <- pad8 c.ofs + 6;
...
module LE_integer_writers : Integer_writers = struct
...
module BE_integer_writers : Integer_writers = struct
  let put_int16 buf ofs v =
...
module LE_integer_readers : Integer_readers = struct
  let get_int16 buf ofs =
    let v0 = get_uint8 buf (ofs + 0) and v1 = get_uint8 buf (ofs + 1) in
    let v = v0 lor (v1 lsl 8) in
    if v land (1 lsl 15) = 0 then v else -1 land lnot 0x7fff lor v
  ...
  let get_int32 buf ofs =
    ...
    Int32.logor
      (Int32.logor (Int32.of_int v0) (Int32.shift_left (Int32.of_int v1) 8))
      (Int32.logor
  ...
  let get_int64 buf ofs =
    ...
    Int64.logor
      (Int64.logor
         (Int64.logor (Int64.of_int v0) (Int64.shift_left (Int64.of_int v1) 8))
         (Int64.logor
...
module BE_integer_readers : Integer_readers = struct
  let get_int16 buf ofs =
    let v1 = get_uint8 buf (ofs + 0) and v0 = get_uint8 buf (ofs + 1) in
    let v = v0 lor (v1 lsl 8) in
    if v land (1 lsl 15) = 0 then v else -1 land lnot 0x7fff lor v
...
module Make_writer (Integer_writers : Integer_writers) = struct
  ...
  let write_basic ptr = function
    ...
    | V.String x -> (
        match OBus_string.validate x with
        | Some error -> raise (Data_error (OBus_string.error_message error))
        | None -> write_string_no_check ptr x )
  ...
  let rec write_array ptr padded_on_8 write_element values =
    ...
    if length < 0 || length > max_array_size then
      raise (Data_error (array_too_big length));
  ...
  and write_single ptr = function
    | V.Basic x -> write_basic ptr x
    | V.Array (t, x) -> write_array ptr (pad8_p t) write_single x
    | V.Byte_array s ->
        write_uint ptr (String.length s);
        write_bytes ptr s
    | V.Dict (tk, tv, x) -> write_array ptr true write_dict_entry x
    | V.Structure x ->
  ...
  let write_field ptr code typ writer = function
    | None -> ()
    | Some value -> write_field_real ptr code typ writer value
  ...
  let write_message byte_order_char msg =
    let { Count.ofs = size; Count.fds } = Count.message msg in
    ...
    let ptr =
      {
        ...
        fds =
          snd
            (FD_set.fold
               (fun fd (n, map) -> (n + 1, FD_map.add fd n map))
               fds (0, FD_map.empty));
      }
    in
    ...
    let code, fields =
      ...
      | Method_call (path, interface, member) ->
          if member = "" then
            raise
              (Data_error "invalid method-call message: field 'member' is empty");
          ( 1,
            {
              rf_path = Some path;
      ...
      | Error (reply_serial, error_name) ->
          if error_name = "" then
            raise
              (Data_error "invalid error message: field 'error-name' is empty");
          ( 3,
            {
              rf_path = None;
              rf_interface = "";
      ...
      | Signal (path, interface, member) ->
          if interface = "" then
            raise
              (Data_error "invalid signal message, field 'interface' is empty");
          if member = "" then
            raise (Data_error "invalid signal message, field 'member' is empty");
          ( 4,
            {
              rf_path = Some path;
              rf_interface = interface;
    ...
    put_uint8 buffer 2
      ( (if msg.flags.no_reply_expected then 1 else 0)
      lor if msg.flags.no_auto_start then 2 else 0 );
...
let string_of_message ?(byte_order = Lwt_io.system_byte_order) msg =
  try
    ...
    | Lwt_io.Little_endian -> LE_writer.write_message 'l' msg
    | Lwt_io.Big_endian -> BE_writer.write_message 'B' msg
  with exn -> raise (map_exn data_error exn)

let write_message oc ?byte_order msg =
  match string_of_message ?byte_order msg with
  | str, [||] -> Lwt_io.write oc str
  | _ ->
      Lwt.fail
        (Data_error "Cannot send a message with file descriptors on a channel")
  ...
  | buf, fds ->
      Lwt_io.atomic
        (fun oc ->
          ...
          Lwt_unix.IO_vectors.append_bytes vec
            (Bytes.unsafe_of_string buf)
            0 len;
          (* Send the file descriptors and the message: *)
          let%lwt n =
            Lwt_unix.Versioned.send_msg_2 writer.w_file_descr vec
              (Array.to_list fds)
          in
          assert (n >= 0 && n <= len);
...
module Make_reader (Integer_readers : Integer_readers) = struct
  ...
  let read_string_no_check ptr =
    ...
    if read_uint8 ptr <> 0 then
      raise (Protocol_error "missing string terminal null byte");
    x
  ...
  let read_signature ptr =
    ...
    let x = read_bytes ptr len in
    if read_uint8 ptr <> 0 then
      raise (Protocol_error "missing signature terminating null byte");
  ...
  let read_vboolean ptr =
    match read_uint ptr with
    | 0 -> V.Boolean false
    | 1 -> V.Boolean true
  ...
  let read_unix_fd ptr =
    ...
    else V.Unix_fd (Array.unsafe_get ptr.fds index)
  ...
  let read_array padded_on_8 read_element ptr =
    ...
    if len < 0 || len > max_array_size then
      raise (Protocol_error (array_too_big len));
  ...
  let rec single_reader = function
    ...
    | T.Variant -> read_variant

  and read_variant ptr =
    match read_signature ptr with
    | [ t ] -> V.variant (single_reader t ptr)
    | s ->
        raise
          (Protocol_error
             (Printf.sprintf
                "variant signature does not contain one single type: %S"
                (OBus_value.string_of_signature s)))
  ...
  let read_field code typ reader ptr =
    match read_signature ptr with
    | [ T.Basic t ] when t = typ -> reader ptr
    | s ->
        raise
          (Protocol_error
             (sprintf
                "invalid header field signature for code %d: %S, should be %S"
                code (string_of_signature s)
                (string_of_signature [ T.Basic typ ])))
  ...
  let read_message buffer get_message =
    ...
    let message_maker =
      match get_uint8 buffer 1 with
      | 1 -> method_call_of_raw
      | 2 -> method_return_of_raw
      | 3 -> error_of_raw
      | 4 -> signal_of_raw
      | n -> raise (Protocol_error (sprintf "unknown message type: %d" n))
    in
    let n = get_uint8 buffer 2 in
    let flags =
      { no_reply_expected = n land 1 = 1; no_auto_start = n land 2 = 2 }
    in
    ...
    get_message total_length (fun ptr pending_fds cont ->
        let fields =
          {
            rf_path = None;
            rf_member = "";
        ...
        while ptr.ofs < limit do
          ...
          | 1 ->
              fields.rf_path <-
                Some (read_field 1 T.Object_path read_object_path ptr)
          | 2 ->
              fields.rf_interface <-
                read_name_field 2 OBus_name.validate_interface ptr
          | 3 ->
              fields.rf_member <-
                read_name_field 3 OBus_name.validate_member ptr
          | 4 ->
              fields.rf_error_name <-
                read_name_field 4 OBus_name.validate_error ptr
          | 5 ->
              fields.rf_reply_serial <-
                Some (read_field 5 T.Uint32 (read4 get_uint32) ptr)
          | 6 ->
              fields.rf_destination <-
                read_name_field 6 OBus_name.validate_bus ptr
          | 7 ->
              fields.rf_sender <- read_name_field 7 OBus_name.validate_bus ptr
          | 8 ->
              fields.rf_signature <- read_field 8 T.Signature read_signature ptr
          | 9 ->
              fields.rf_unix_fds <- read_field 9 T.Uint32 (read4 get_uint) ptr
          | _ -> ignore (read_variant ptr)
        ...
        | None ->
            if fields.rf_unix_fds <> Array.length ptr.fds then
              raise
                (Protocol_error
                   (sprintf
                      "invalid number of file descriptor, %d expected, %d \
                       received"
                      fields.rf_unix_fds (Array.length ptr.fds)))
        | Some (consumed, queue) ->
            ptr.fds <-
              Array.init fields.rf_unix_fds (fun i ->
        ...
        if ptr.ofs < ptr.max then
          raise (Protocol_error "junk bytes after message");
        cont
          {
            flags;
  ...
  try%lwt
    Lwt_io.atomic
      (fun ic ->
        ...
        | ch -> raise (Protocol_error (invalid_byte_order ch)) ) buffer
          (fun length f ->
            ...
            let buffer = Bytes.unsafe_to_string buffer in
            f
              { buf = buffer; ofs = 0; max = length; fds = [||] }
              None Lwt.return))
      ic
  with exn -> raise (map_exn protocol_error exn)

let message_of_string buffer fds =
  if String.length buffer < 16 then
    invalid_arg "OBus_wire.message_of_string: buffer too small";
  try
    ( match get_char buffer 0 with
    | 'l' -> LE_reader.read_message
    | 'B' -> BE_reader.read_message
    | ch -> raise (Protocol_error (invalid_byte_order ch)) ) buffer
      (fun length f ->
        if length <> String.length buffer then
          raise (Protocol_error "invalid message size");
        f { buf = buffer; ofs = 16; max = length; fds } None (fun x -> x))
  with exn -> raise (map_exn protocol_error exn)
...
let close_reader reader =
  ...
  let%lwt () =
    Lwt_list.iter_p
      (fun fd ->
        try Lwt_unix.close (Lwt_unix.of_unix_file_descr ~set_flags:false fd)
...
let reader fd =
  ...
  {
    r_channel =
      Lwt_io.make ~mode:Lwt_io.input (fun buf ofs len ->
          let%lwt n, fds =
      ...
      (fun ic ->
        ...
        | ch -> raise (Protocol_error (invalid_byte_order ch)) ) buffer
          (fun length f ->
            ...
            f
              { buf = buffer; ofs = 0; max = length; fds = [||] }
              (Some (consumed_fds, reader.r_pending_fds))
              Lwt.return))
      reader.r_channel
  with exn ->
    let%lwt () =
...
let get_message_size buf ofs =
  let unsafe_get_uint map_ofs i =
    ...
    Char.code v0
    lor (Char.code v1 lsl 8)
    lor (Char.code v2 lsl 16)
    lor (Char.code v3 lsl 24)
  in
  if ofs < 0 || ofs + 16 >= String.length buf then
    ...
    (* Byte-order *)
    let map_ofs =
      match String.unsafe_get buf ofs with
      | 'l' -> fun i -> i
      | 'B' -> fun i -> 3 - i
