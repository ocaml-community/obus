...
let make connection name =
  ...
  (* If the connection is a peer-to-peer connection, act as if there
     is no owner *)
  if (running.running_name = None ||
      (* If it is a unique name and the peer has already exited, then
         there is nothing to do *)
      (OBus_name.is_unique name && OBus_cache.mem running.running_exited_peers name)) then
    return {
      name = name;
    ...
    lwt name_resolver =
      match try Some(Name_map.find name running.running_resolvers) with Not_found -> None with
        | Some name_resolver ->
            (* add a reference to the resolver *)
            name_resolver.resolver_ref_count <- name_resolver.resolver_ref_count + 1;
            begin
              match name_resolver.resolver_state with
        ...
        | None ->
            ...
            let name_resolver = {
              ...
              resolver_set = set;
              resolver_ref_count = 1;
              resolver_match_rule = match_rule;
              resolver_state = Resolver_init(init_waiter, init_wakener);
            } in

            (* Immediatly add the resolver to be sure no other thread
               will do it: *)
            running.running_resolvers <- Name_map.add name name_resolver running.running_resolvers;
            ...
            try_lwt
              ...
              name_resolver.resolver_set owner;
              match name_resolver.resolver_state with
                | Resolver_init(waiter, wakener) ->
                    name_resolver.resolver_state <- Resolver_running;
            ...
            with exn ->
              match name_resolver.resolver_state with
                | Resolver_init(waiter, wakener) ->
                    running.running_resolvers <- Name_map.remove name running.running_resolvers;
    ...
    let disable = lazy(
      name_resolver.resolver_ref_count <- name_resolver.resolver_ref_count - 1;
      if name_resolver.resolver_ref_count = 0 then
        match connection#get with
          | Running running ->
              running.running_resolvers <- Name_map.remove name running.running_resolvers;
              React.S.stop name_resolver.resolver_owner;
              OBus_private_bus.remove_match connection name_resolver.resolver_match_rule
          | Crashed _ ->
              return ()
      else
        return ()
    ...
    return {
      name = name;
      owner = Lwt_signal.with_finaliser (finalise disable) name_resolver.resolver_owner;
      disable = disable;
    }
