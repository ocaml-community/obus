...
type resolver = {
  ...
  set_owner : OBus_name.bus -> unit; (* Sets the owner. *)
}
...
let finalise remove _ = ignore (Lazy.force remove)

let has_exited peer_name info =
  let rec loop index =
    if index = cache_size then false
    else if info.exited.(index) = peer_name then true
    else loop (index + 1)
  in
  loop 0
...
let get_name_owner connection name =
  try%lwt
    OBus_connection.method_call ~connection ~destination:OBus_protocol.bus_name
      ~path:OBus_protocol.bus_path ~interface:OBus_protocol.bus_interface
      ~member:"GetNameOwner"
  ...
  with
  | exn
  when OBus_error.name exn = "org.freedesktop.DBus.Error.NameHasNoOwner"
  ->
    Lwt.return ""
...
let update_mapping info message =
  ...
  | {
   ...
   body =
     [
       V.Basic (V.String name);
       V.Basic (V.String old_owner);
       V.Basic (V.String new_owner);
     ];
  } ->
      if
        OBus_name.is_unique name && new_owner = "" && not (has_exited name info)
      then (
        (* Remember that the peer has exited: *)
        info.exited.(info.exited_index) <- name;
        info.exited_index <- (info.exited_index + 1) mod cache_size );
      ( match
          try Lwt.state (String_map.find name info.resolvers)
          with Not_found -> Sleep
        with
      | Return (resolver, switch) -> resolver.set_owner new_owner
      | Fail _ | Sleep ->
...
let make ?switch connection name =
  ...
  let info =
    ...
    | None ->
        ...
        let _ =
          Lwt_sequence.add_l (update_mapping info)
  ...
  if OBus_name.is_unique name && has_exited name info then
    ...
    let%lwt resolver, export_switch =
      match
        try Some (String_map.find name info.resolvers) with Not_found -> None
      with
      | Some thread -> thread
      | None -> (
          ...
          try%lwt
            let%lwt () =
              OBus_match.export ~switch:export_switch connection
                (OBus_match.rule ~typ:`Signal ~sender:OBus_protocol.bus_name
                   ~interface:OBus_protocol.bus_interface
                   ~member:"NameOwnerChanged" ~path:OBus_protocol.bus_path
                   ~arguments:
                     (OBus_match.make_arguments
                        [ (0, OBus_match.AF_string name) ])
                   ())
            in
          ...
          with exn ->
            let%lwt () =
              Lwt_log.warning_f ~section ~exn
                "failed to disable resolver for name %S" name
            in
            Lwt.fail exn )
    in
