...
let default_flags = { no_reply_expected = false; no_auto_start = false }
...
let make ?(flags = default_flags) ?(serial = 0l) ?(sender = "")
    ?(destination = "") ~typ body =
  { flags; serial; typ; destination; sender; body }

let method_call ?flags ?serial ?sender ?destination ~path ?(interface = "")
    ~member body =
  make ?flags ?serial ?sender ?destination
...
let invalid_reply ~method_call ~expected_signature ~method_return =
  ...
  | ( { typ = Method_call (path, interface, member) },
      { typ = Method_return _; body } ) ->
      Invalid_reply
        (Printf.sprintf
           "unexpected signature for the reply to the method %S on interface \
            %S, expected: %S, got: %S"
           member interface
           (OBus_value.string_of_signature expected_signature)
           (OBus_value.string_of_signature (OBus_value.V.type_of_sequence body)))
  | _ -> invalid_arg "OBus_message.invalid_reply"
  ...
  fprintf pp
    ...
    (fun pp -> function
      ...
      | Error (reply_serial, error_name) ->
          fprintf pp "error@\nreply_serial = %ld@\nerror_name = %S" reply_serial
            error_name
      | Signal (path, interface, member) ->
          fprintf pp "signal@\npath = %S@\ninterface = %S@\nmember = %S"
            (OBus_path.to_string path) interface member)
    message.typ message.sender message.destination
    (string_of_signature (V.type_of_sequence message.body))
    T.print_sequence
    (V.type_of_sequence message.body)
