...
type 'a t = {
  ...
  map :
    (OBus_context.t * OBus_path.t * OBus_value.V.sequence) event ->
    (OBus_context.t * 'a) event;
  (* The function which maps the event into an event holding values of
     type ['a]. *)
  filters : OBus_match.arguments;
  (* Argument filters. *)
  match_rule : bool; (* Whether the managed mode for the match rule is enabled *)
}
    ...
    Some
      ( context,
        OBus_value.C.cast_sequence
          (OBus_value.arg_types (OBus_member.Signal.args signal))
          body )
  with OBus_value.C.Signature_mismatch ->
    ignore
      (Lwt_log.error_f ~section
         "failed to cast signal from %S, interface %S, member %S with \
          signature %S to %S"
...
let cast_any signal (context, path, body) =
  ...
  | Some (context, v) ->
      Some (context, (OBus_proxy.make (OBus_context.sender context) path, v))
  | None -> None
...
let map_event f sd = { sd with map = (fun event -> f (sd.map event)) }
...
let map f sd =
  {
    ...
    map =
      (fun event ->
        E.map (fun (context, value) -> (context, f value)) (sd.map event));
  }
...
let map_with_context f sd =
  {
    ...
    map =
      (fun event ->
        E.map
          (fun (context, value) -> (context, f context value))
          (sd.map event));
  }
...
let with_context sd =
  {
    ...
    map =
      (fun event ->
        E.map
          (fun (context, value) -> (context, (context, value)))
          (sd.map event));
  }

let with_filters filters sd = { sd with filters }

let with_match_rule match_rule sd = { sd with match_rule }

(* +-----------------------------------------------------------------+
   | Signals dispatching                                             |
   +-----------------------------------------------------------------+ *)

module Signal_map = Map.Make (struct
...
type info = {
  mutable senders :
    (OBus_context.t * OBus_path.t * OBus_value.V.sequence -> unit)
    Lwt_sequence.t
    Signal_map.t;
}
...
let dispatch connection info message =
  ...
  | OBus_message.Signal (path, interface, member) ->
      ( match
          try Some (Signal_map.find (Some path, interface, member) info.senders)
          with Not_found -> None
        with
      | Some senders ->
          Lwt_sequence.iter_l
            (fun send ->
              try
                send
                  ( OBus_context.make connection message,
                    path,
      ...
      | None -> () );
      ( match
          try Some (Signal_map.find (None, interface, member) info.senders)
          with Not_found -> None
        with
      | Some senders ->
          Lwt_sequence.iter_l
...
let finalise disconnect _ = ignore (Lazy.force disconnect)

let key = OBus_connection.new_key ()

let connect ?switch sd =
  Lwt_switch.check switch;
  let connection = OBus_peer.connection sd.peer
  and name = OBus_peer.name sd.peer in
  ...
  try%lwt
    ...
    let%lwt () =
      if sd.match_rule then
        OBus_match.export ~switch:resources_switch connection
          (OBus_match.rule ~typ:`Signal ~sender:name ?path:sd.path
             ~interface:sd.interface ~member:sd.member ())
      else Lwt.return ()
    (* Plus the resolver if needed: *)
    and owner_option =
      if OBus_connection.name connection <> "" && name <> "" then
        if OBus_name.is_unique name then Lwt.return (Some (S.const name))
        else
          let%lwt owner =
            OBus_resolver.make ~switch:resources_switch connection name
          in
    ...
    let info =
      ...
      | Some info -> info
      | None ->
          let info = { senders = Signal_map.empty } in
          OBus_connection.set connection key (Some info);
          let _ =
            Lwt_sequence.add_l (dispatch connection info)
    ...
    let senders =
      match
        try
          Some (Signal_map.find (sd.path, sd.interface, sd.member) info.senders)
        with Not_found -> None
      with
      ...
      | None ->
          ...
          info.senders <-
            Signal_map.add
              (sd.path, sd.interface, sd.member)
    ...
    let event =
      E.filter
        (fun (context, path, body) ->
          ...
          | Some owner
            when S.value owner <> OBus_peer.name (OBus_context.sender context)
            ->
              false
          | _ -> OBus_match.match_values sd.filters body)
        event
    in
    let disconnect =
      lazy
        ( try%lwt
          ...
          with exn ->
            let%lwt () =
              Lwt_log.warning_f ~section ~exn
                "failed to disconnect signal \"%s.%s\" of object \"%s\" from \
                 \"%s\""
                sd.interface sd.member
                ( match sd.path with
    ...
    in
    let event =
      E.with_finaliser (finalise disconnect) (E.map snd (sd.map event))
    in
    let%lwt () =
