...
let make ~connection ~name = { connection; name }

let anonymous c = { connection = c; name = "" }

let ping peer =
  let%lwt reply, () =
    OBus_connection.method_call_with_message ~connection:peer.connection
      ~destination:OBus_protocol.bus_name ~path:[]
      ~interface:"org.freedesktop.DBus.Peer" ~member:"Peer"
      ~i_args:OBus_value.C.seq0 ~o_args:OBus_value.C.seq0 ()
  in
  Lwt.return { peer with name = OBus_message.sender reply }

let get_machine_id peer =
  let%lwt mid =
    OBus_connection.method_call ~connection:peer.connection
      ~destination:OBus_protocol.bus_name ~path:[]
      ~interface:"org.freedesktop.DBus.Peer" ~member:"GetMachineId"
      ~i_args:OBus_value.C.seq0
      ~o_args:(OBus_value.C.seq1 OBus_value.C.basic_string)
      ()
  in
  try Lwt.return (OBus_uuid.of_string mid) with exn -> Lwt.fail exn
...
let wait_for_exit peer =
  ...
  | name ->
      ...
      if S.value owner = "" then Lwt_switch.turn_off switch
      else
        (let%lwt _ = E.next (E.filter (( = ) "") (S.changes owner)) in
         Lwt.return ())
          [%lwt.finally Lwt_switch.turn_off switch]
