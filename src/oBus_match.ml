...
let rule ?typ ?(sender = "") ?(interface = "") ?(member = "") ?path
    ?(destination = "") ?(arguments = []) ?eavesdrop () =
  { typ; sender; interface; member; path; destination; arguments; eavesdrop }
...
let rec insert_sorted num filter = function
  ...
  | (num', _) :: rest when num' = num -> (num, filter) :: rest
  | (num', _) :: rest as l -> (num, filter) :: l
    ...
    (fun l (num, filter) ->
      if num < 0 || num > 63 then
        Printf.ksprintf invalid_arg
          "OBus_match.arguments_of_list: invalid argument number '%d': it must \
           be in the rane [1..63]"
          num
      else insert_sorted num filter l)
    [] list
...
let string_of_rule mr =
  ...
  List.iter
    (fun (n, filter) ->
      !coma ();
      match filter with
      | AF_string str -> Printf.bprintf buf "arg%d='%s'" n str
      | AF_string_path str -> Printf.bprintf buf "arg%dpath='%s'" n str
      | AF_namespace str -> Printf.bprintf buf "arg%dnamespace='%s'" n str)
    mr.arguments;
  ( match mr.eavesdrop with
  | None -> ()
...
let () =
  Printexc.register_printer (function
    | Parse_failure (str, pos, reason) ->
        Some
          (Printf.sprintf
             "failed to parse D-Bus matching rule %S, at position %d: %s" str
             pos reason)
    | _ -> None)
...
let rule_of_string str =
  try
    ...
    let check pos validate value =
      ...
      | None -> ()
      | Some err -> raise (Fail (pos, OBus_string.error_message err))
    in
    let mr =
      {
        typ = None;
    ...
    List.fold_left
      (fun mr (pos, key, value) ->
                ...
                Some
                  ...
                  | _ ->
                      raise
                        (Fail
                           ( pos,
                             Printf.sprintf "invalid message type (%s)" value ))
                  );
            }
        | "sender" ->
        ...
        | "path" -> (
            try { mr with path = Some (OBus_path.of_string value) }
        ...
        | "eavesdrop" -> (
            ...
            | _ ->
                raise
                  (Fail
                     ( pos,
                       Printf.sprintf "invalid value for eavesdrop (%s)" value
                     )) )
        | _ -> (
            ...
            | None -> raise (Fail (pos, Printf.sprintf "invalid key (%s)" key))
            ))
      mr l
  with Fail (pos, msg) -> raise (Parse_failure (str, pos, msg))

(* +-----------------------------------------------------------------+
   | Matching                                                        |
   +-----------------------------------------------------------------+ *)

let match_key matcher value =
  match matcher with None -> true | Some value' -> value = value'

let match_string matcher value =
  match matcher with "" -> true | value' -> value = value'
...
let rec match_arguments num matcher arguments =
  match matcher with
  | [] -> true
  | (num', filter) :: rest -> match_arguments_aux num num' filter rest arguments

and match_arguments_aux num num' filter matcher arguments =
  ...
  | OBus_value.V.Basic (OBus_value.V.Object_path value) :: rest ->
      ...
      | AF_string_path str ->
          ...
          || (ends_with_slash str && starts_with value str)
          || (ends_with_slash value && starts_with str value)
      | AF_namespace _ -> false )
      && match_arguments (num + 1) matcher rest
  | _ -> false

let match_values filters values = match_arguments 0 filters values

let match_message mr msg =
  ( match (OBus_message.typ msg, mr.typ) with
  | ( OBus_message.Method_call (path, interface, member),
      (Some `Method_call | None) ) ->
      match_key mr.path path
      && match_string mr.interface interface
      && match_string mr.member member
  | OBus_message.Method_return serial, (Some `Method_return | None) -> true
  | OBus_message.Signal (path, interface, member), (Some `Signal | None) ->
      match_key mr.path path
      && match_string mr.interface interface
      && match_string mr.member member
  | OBus_message.Error (serial, name), (Some `Error | None) -> true
  | _ -> false )
...
let rec compare_arguments acc l1 l2 =
  ...
  | acc, [], [] -> acc
  | (Less_general | Equal), _ :: _, [] -> Less_general
  | (More_general | Equal), [], _ :: _ -> More_general
  | acc, (pos1, filter1) :: rest1, (pos2, filter2) :: rest2 ->
      if pos1 = pos2 && filter1 = filter2 then compare_arguments acc rest1 rest2
...
let compare_option acc x1 x2 =
  if x1 = x2 then acc
    ...
    | (Less_general | Equal), Some _, None -> Less_general
    | (More_general | Equal), None, Some _ -> More_general
    | _ -> raise Exit
...
let compare_string acc x1 x2 =
  if x1 = x2 then acc
    ...
    match (acc, x1, x2) with
    | (Less_general | Equal), x, "" when x <> "" -> Less_general
    | (More_general | Equal), "", x when x <> "" -> More_general
    | _ -> raise Exit

let compare_rules r1 r2 =
  try
    if r1.typ = r2.typ then
      let acc = Equal in
      ...
      if r1.eavesdrop = r2.eavesdrop then acc
      else
        match (acc, r1.eavesdrop, r2.eavesdrop) with
        | _, None, Some false -> acc
        | _, Some false, None -> acc
        | (Less_general | Equal), (None | Some false), Some true -> Less_general
        | (More_general | Equal), Some true, (None | Some false) -> More_general
        | _ -> Incomparable
    else Incomparable
  with Exit -> Incomparable
...
type info = {
  ...
  mutex : Lwt_mutex.t; (* Mutex to prevent concurrent modifications of rules *)
}
...
let rec insert_rule rule rules =
  ...
  | rule' :: rest -> (
      ...
      | Incomparable -> rule' :: insert_rule rule rest
      | Equal | Less_general -> rules
      | More_general -> rule :: rest )

let do_export info rule_string =
  let%lwt () =
    OBus_connection.method_call ~connection:info.connection
      ~destination:OBus_protocol.bus_name ~path:OBus_protocol.bus_path
      ~interface:OBus_protocol.bus_interface ~member:"AddMatch"
      ~i_args:(OBus_value.C.seq1 OBus_value.C.basic_string)
      ~o_args:OBus_value.C.seq0 rule_string
  in
...
let do_remove info rule_string =
  ...
  try%lwt
    OBus_connection.method_call ~connection:info.connection
      ~destination:OBus_protocol.bus_name ~path:OBus_protocol.bus_path
      ~interface:OBus_protocol.bus_interface ~member:"RemoveMatch"
      ~i_args:(OBus_value.C.seq1 OBus_value.C.basic_string)
      ~o_args:OBus_value.C.seq0 rule_string
  with exn -> (
    match OBus_error.name exn with
    | "org.freedesktop.DBus.Error.MatchRuleNotFound" ->
        Lwt_log.info_f ~section "rule %S does not exists on the message bus"
          rule_string
    | _ -> Lwt.fail exn )

(* Commits rules changes on the message bus: *)
let commit info =
  Lwt_mutex.with_lock info.mutex (fun () ->
      (* Computes the set of most general rules: *)
      let rules =
        List.fold_left (fun acc rule -> insert_rule rule acc) [] info.rules
      in
      (* Turns them into a set of strings: *)
      let rules =
        List.fold_left
          (fun acc rule -> String_set.add (string_of_rule rule) acc)
      ...
      let threads =
        String_set.fold
          (fun rule acc -> do_export info rule :: acc)
          new_rules threads
      in
      let threads =
        String_set.fold
          (fun rule acc -> do_remove info rule :: acc)
          old_rules threads
      in
      Lwt.join threads)
