...
let proxy bus =
  OBus_proxy.make
    (OBus_peer.make bus OBus_protocol.bus_name)
    OBus_protocol.bus_path

let exit_on_disconnect = function
  | OBus_wire.Protocol_error msg ->
      ignore
        (Lwt_log.error_f ~section
           "the D-Bus connection with the message bus has been closed due to a \
            protocol error: %s"
           msg);
      exit 1
  ...
  | exn ->
      ignore
        (Lwt_log.error ~section ~exn
           "the D-Bus connection with the message bus has been closed due to \
...
let register_connection connection =
  ...
  | None ->
      ...
      let _ =
        Lwt_sequence.add_l (update_names info)
...
let session_bus =
  lazy
    ( try%lwt
      ...
      with exn ->
        let%lwt () =
          Lwt_log.warning ~exn ~section
            "Failed to open a connection to the session bus"
        in
        Lwt.fail exn )
...
let session ?switch () =
  ...
  let%lwt () =
    Lwt_switch.add_hook_or_exec switch (fun () -> OBus_connection.close bus)
  in
  Lwt.return bus
...
let system ?switch () =
  Lwt_switch.check switch;
  let%lwt bus =
    Lwt_mutex.with_lock system_bus_mutex (fun () ->
        match !system_bus_state with
        | Some bus when S.value (OBus_connection.active bus) -> Lwt.return bus
        | _ -> (
            try%lwt
            ...
            with exn ->
              let%lwt () =
                Lwt_log.warning ~exn ~section
                  "Failed to open a connection to the system bus"
              in
              Lwt.fail exn ))
  in
  let%lwt () =
...
let hello bus = OBus_method.call m_Hello (proxy bus) ()

type request_name_result = type_request_name_result

let request_name bus ?(allow_replacement = false) ?(replace_existing = false)
    ?(do_not_queue = false) name =
  let flags = [] in
  let flags =
    if allow_replacement then `Allow_replacement :: flags else flags
  in
  ...
  OBus_method.call m_RequestName (proxy bus)
    (name, cast_request_name_flags flags)
...
let start_service_by_name bus name =
  OBus_method.call m_StartServiceByName (proxy bus) (name, 0l)
  >|= make_start_service_by_name_result

let name_has_owner bus name = OBus_method.call m_NameHasOwner (proxy bus) name

let list_names bus = OBus_method.call m_ListNames (proxy bus) ()

let list_activatable_names bus =
  OBus_method.call m_ListActivatableNames (proxy bus) ()

let get_name_owner bus name = OBus_method.call m_GetNameOwner (proxy bus) name
...
let get_connection_unix_process_id bus name =
  OBus_method.call m_GetConnectionUnixProcessID (proxy bus) name
  >|= Int32.to_int
...
let reload_config bus = OBus_method.call m_ReloadConfig (proxy bus) ()

let get_id bus = OBus_method.call m_GetId (proxy bus) () >|= OBus_uuid.of_string

let name_owner_changed bus = OBus_signal.make s_NameOwnerChanged (proxy bus)

let name_lost bus = OBus_signal.make s_NameLost (proxy bus)

let name_acquired bus = OBus_signal.make s_NameAcquired (proxy bus)
