...
let introspect obj =
  Array.fold_right
    (fun interface acc ->
      ...
      let members =
        Array.fold_right
          (fun member acc -> introspect_property member :: acc)
          interface.i_properties members
      in
      let members =
        Array.fold_right
          (fun member acc -> introspect_signal member :: acc)
          interface.i_signals members
      in
      let members =
        Array.fold_right
          (fun member acc -> introspect_method member :: acc)
          interface.i_methods members
      in
...
let binary_search compare key array =
  let rec loop a b =
    if a = b then -1
      ...
      if cmp = 0 then middle
      else if cmp < 0 then loop a middle
      else loop (middle + 1) b
  in
  loop 0 (Array.length array)

let compare_interface name interface = String.compare name interface.i_name
...
let unknown_method interface member arguments =
  Lwt.fail
    (OBus_error.Unknown_method
       (Printf.sprintf
          "Method %S with signature %S on interface %S does not exist" member
           ...
           (Printf.sprintf
              ...
              (OBus_value.string_of_signature
                 (OBus_value.C.type_sequence
                    (OBus_value.arg_types (OBus_member.Method.i_args M.info))))))
  in
  Lwt.with_value OBus_context.key (Some context) (fun () ->
      let%lwt reply = M.handler obj arguments in
      Lwt.return
        (OBus_value.C.make_sequence
           (OBus_value.arg_types (OBus_member.Method.o_args M.info))
           reply))
  ...
  if interface = "" then
    let rec loop i =
      if i = Array.length obj.interfaces then
        ...
        match
          binary_search compare_method member obj.interfaces.(i).i_methods
        with
        | -1 -> loop (i + 1)
        | index ->
    ...
    | -1 -> unknown_method interface member arguments
    | index -> (
        let interface = obj.interfaces.(index) in
        match binary_search compare_method member interface.i_methods with
        | -1 -> unknown_method interface.i_name member arguments
        | index -> execute interface.i_methods.(index) context obj arguments )
...
let send_reply context value =
  try%lwt
    ...
    OBus_connection.send_message
      (OBus_context.connection context)
      {
             ...
             | None -> (
                 ...
                 | Some (path, dynamic) -> (
                     ...
                     let%lwt result =
                       ...
                       with exn -> Lwt.return (`Failure exn)
                     in
                     match result with
                     | `Success obj ->
                         dispatch context obj interface member
                           (OBus_message.body message)
                     | `Failure Not_found -> Lwt.fail No_such_object
                     | `Failure exn ->
                         let%lwt () =
                           Lwt_log.error ~section ~exn
                             "dynamic object handler failed with"
                         in
         ...
         | No_such_object -> (
             ...
             | ("" | "org.freedesktop.DBus.Introspectable"), "Introspect", [] ->
                 ...
                 OBus_introspect.output
                   ...
                   ([], children info path);
                 send_reply context
                   [ OBus_value.V.basic_string (Buffer.contents buffer) ]
             | _ ->
                 send_error context
                   (OBus_error.Unknown_object
                      (Printf.sprintf "Object %S does not exists"
             ...
             let%lwt () =
               if OBus_error.name exn = OBus_error.ocaml then
                 ...
                 Lwt_log.error_f ~section ~exn
                   "method call handler for method %S on interface %S failed \
                    with"
                   member interface
               else Lwt.return ()
             in
...
let cleanup connection info =
  ...
  Path_map.iter
    (fun path static ->
      let module M = (val static : Static) in
      M.obj.set_exports
        (Connection_set.remove connection (S.value M.obj.exports)))
    info.statics
...
let get_info connection =
  ...
  | None ->
      ...
      let _ =
        Lwt_sequence.add_r
          (handle_message connection info)
          (OBus_connection.incoming_filters connection)
      in
      info.watcher <-
        E.map
          (fun state -> cleanup connection info)
          (E.once (S.changes (OBus_connection.active connection)));
      info

let remove connection obj =
  let exports = S.value obj.exports in
  if Connection_set.mem connection exports then (
    ( if S.value (OBus_connection.active connection) then
      match OBus_connection.get connection key with
      | Some info -> info.statics <- Path_map.remove obj.path info.statics
      | None -> () );
...
let remove_by_path connection path =
  if S.value (OBus_connection.active connection) then
    ...
    | Some info -> (
        ...
        match
          try Some (Path_map.find path info.statics) with Not_found -> None
        with
        | Some static ->
            let module M = (val static : Static) in
...
let export (type d) connection obj =
  if obj.data = None then
    ...
    let exports = S.value obj.exports in
    if not (Connection_set.mem connection exports) then (
      let info = get_info connection in
      let () =
        (* Remove any object registered under the same path: *)
        match
          try Some (Path_map.find obj.path info.statics)
          with Not_found -> None
        with
        | Some static ->
            let module M = (val static : Static) in
...
let destroy obj =
  Connection_set.iter
    (fun connection -> remove connection obj)
    (S.value obj.exports)
...
let emit obj ~interface ~member ?peer typ x =
  ...
  | None, None ->
      ...
      Lwt.join
        (Connection_set.fold
           (fun connection l ->
             OBus_connection.send_message connection signal :: l)
           (S.value obj.exports) [])
...
let notify_properties_change (type d) obj interface_name changed index =
  ...
  try%lwt
    !(obj.properties_changed) interface_name
      (String_map.fold
         (fun name value_opt acc -> (name, value_opt) :: acc)
         members [])
  with exn ->
...
let handle_property_change obj index info value_opt =
  ...
  obj.changed.(index) <-
    String_map.add
      ...
      value_opt obj.changed.(index);
  if empty then
    ignore
      (notify_properties_change obj
         (OBus_member.Property.interface info)
         obj.changed index)

let handle_property_change_true (type d v) obj interface_index prop value =
  let module P = (val prop : Property_info with type obj = d t and type typ = v)
  in
  let value =
    OBus_value.C.make_single (OBus_member.Property.typ P.info) value
  in
  handle_property_change obj interface_index P.info (Some value)

let handle_property_change_invalidates (type d v) obj interface_index prop value
    =
  let module P = (val prop : Property_info with type obj = d t and type typ = v)
  in
...
let get_emits_changed_signal annotations =
  try
    ...
    | value ->
        ignore
          (Lwt_log.warning_f
             "invalid value(%S) for annotation %S. Using default(\"true\")"
             value OBus_introspect.emits_changed_signal);
        Esc_true
  with Not_found -> Esc_default
            ...
            let monitor =
              let esc_prop =
                get_emits_changed_signal
                  (OBus_member.Property.annotations P.info)
              and esc_intf =
                get_emits_changed_signal obj.interfaces.(i).i_annotations
              in
              let info =
                (module P : Property_info
                  with type obj = d t
                   and type typ = P.typ )
              in
              match (esc_prop, esc_intf) with
              | Esc_false, _ | Esc_default, Esc_false -> E.never
              | Esc_true, _ | Esc_default, (Esc_default | Esc_true) ->
                  E.map
                    (handle_property_change_true obj i info)
                    (S.changes signal)
              | Esc_invalidates, _ | Esc_default, Esc_invalidates ->
                  E.map
                    (handle_property_change_invalidates obj i info)
                    (S.changes signal)
          end in
...
let method_info (type d i o) info f =
  let module M = struct
    type obj = d t
...
let property_r_info (type d i a) info signal =
  let module M = struct
    type obj = d t
...
let property_w_info (type d i a) info set =
  let module M = struct
    type obj = d t
...
let property_rw_info (type d i a) info signal set =
  let module M = struct
    type obj = d t
...
let compare_methods (type d) m1 m2 =
  ...
  String.compare
    (OBus_member.Method.member M1.info)
    (OBus_member.Method.member M2.info)
...
let compare_signals (type d) s1 s2 =
  ...
  String.compare
    (OBus_member.Signal.member S1.info)
    (OBus_member.Signal.member S2.info)
...
let compare_properties (type d) p1 p2 =
  ...
  String.compare
    (OBus_member.Property.member P1.info)
    (OBus_member.Property.member P2.info)

let make_interface ~name ?(annotations = []) ?(methods = []) ?(signals = [])
    ?(properties = []) () =
...
let process_interfaces interfaces =
  let rec uniq = function
    | iface :: iface' :: rest when iface.i_name = iface'.i_name ->
        uniq (iface :: rest)
    | iface :: rest -> iface :: uniq rest
    | [] -> []
  and compare i1 i2 = String.compare i1.i_name i2.i_name in
  Array.of_list (uniq (List.stable_sort compare interfaces))

let add_interfaces obj interfaces =
  obj.interfaces <-
    process_interfaces (interfaces @ Array.to_list obj.interfaces);
  generate obj
...
let remove_interfaces_by_names obj names =
  obj.interfaces <-
    Array.of_list
      (List.filter
         (fun iface -> not (List.mem iface.i_name names))
         (Array.to_list obj.interfaces));
  generate obj

let remove_interfaces obj interfaces =
  remove_interfaces_by_names obj
    (List.map (fun iface -> iface.i_name) interfaces)
         ...
         let info =
           {
             ...
             Method.o_args =
               OBus_value.arg1 (Some "result", OBus_value.C.basic_string);
             Method.annotations = [];
           }
    ...
    [|
      (let module M = struct
         ...
         let info =
           {
             ...
             Method.o_args = OBus_value.arg1 (Some "value", OBus_value.C.variant);
             Method.annotations = [];
           }
         ...
         let handler obj (interface, member) =
           ...
           | -1 ->
               Lwt.fail
                 (OBus_error.Unknown_interface
                    (Printf.sprintf "Interface %S does not exists" interface))
           | i -> (
               match
                 binary_search compare_property member
                   obj.interfaces.(i).i_properties
               with
               | -1 ->
                   Lwt.fail
                     (OBus_error.Unknown_property
                        (Printf.sprintf
                           "Property %S on interface %S does not exists" member
                           interface))
               | j -> (
                   ...
                   | None ->
                       Lwt.fail
                         (OBus_error.Failed
                            (Printf.sprintf
                               "Property %S on interface %S is not readable"
                               member interface)) ) )
       end in
      (module M : Method_info with type obj = d t));
      (let module M = struct
         ...
         let info =
           {
             ...
             Method.i_args =
               OBus_value.arg1 (Some "interface", OBus_value.C.basic_string);
             Method.o_args =
               OBus_value.arg1
                 ( Some "values",
                   ...
                   Lwt.fail
                     (OBus_error.Unknown_property
                        (Printf.sprintf
                           ...
                           interface))
               | j -> (
                   let module P = ( val obj.interfaces.(i).i_properties.(j)
                                      : Property_info
                                      with type obj = d t )
                   in
                   match P.set with
                   | Some f -> (
                       match
                         try
                           `Success
                             (OBus_value.C.cast_single (Property.typ P.info)
                                value)
                         with exn -> `Failure exn
                       with
                       | `Success value -> f obj value
                       | `Failure OBus_value.C.Signature_mismatch ->
                           Lwt.fail
                             (OBus_error.Failed
                                (Printf.sprintf
                                   "invalid type(%S) for property %S on \
                                    interface %S, should be %S"
                                   (OBus_value.string_of_signature
                   ...
                   | None ->
                       Lwt.fail
                         (OBus_error.Property_read_only
                            (Printf.sprintf
                               "property %S on interface %S is not writable"
                               member interface)) ) )
       end in
      (module M : Method_info with type obj = d t));
    |]
...
let properties_changed obj interface values =
  emit obj ~interface:"org.freedesktop.DBus.Properties"
    ~member:"PropertiesChanged"
    (OBus_value.C.seq3 OBus_value.C.basic_string
       (OBus_value.C.dict OBus_value.C.string OBus_value.C.variant)
       (OBus_value.C.array OBus_value.C.basic_string))
    ( interface,
      OBus_util.filter_map
        (function name, Some value -> Some (name, value) | name, None -> None)
        values,
      OBus_util.filter_map
        (function name, Some value -> None | name, None -> Some name)
        values )

let make ?owner ?(common = true) ?(interfaces = []) path =
  let interfaces =
    if common then introspectable () :: properties () :: interfaces
    else interfaces
  in
  let exports, set_exports =
    S.create ~eq:Connection_set.equal Connection_set.empty
  in
  let obj =
    {
      path;
  ...
  (obj.properties_changed :=
     fun name values -> properties_changed obj name values);
  obj

let attach obj data =
  match obj.data with
  | Some _ -> failwith "OBus_object.attach: object already contains attached"
  | None -> (
      obj.data <- Some data;
      generate obj;
