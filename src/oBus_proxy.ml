...
let call proxy ~interface ~member ~i_args ~o_args args =
  OBus_connection.method_call ~connection:proxy.peer.connection
    ~destination:proxy.peer.name ~path:proxy.path ~interface ~member ~i_args
    ~o_args args

let call_with_context proxy ~interface ~member ~i_args ~o_args args =
  let%lwt msg, result =
    OBus_connection.method_call_with_message ~connection:proxy.peer.connection
      ~destination:proxy.peer.name ~path:proxy.path ~interface ~member ~i_args
      ~o_args args
  in
...
let call_no_reply proxy ~interface ~member ~i_args args =
  OBus_connection.method_call_no_reply ~connection:proxy.peer.connection
    ~destination:proxy.peer.name ~path:proxy.path ~interface ~member ~i_args
    args
...
let introspect proxy =
  let%lwt str =
    call proxy ~interface:"org.freedesktop.DBus.Introspectable"
      ~member:"Introspect" ~i_args:OBus_value.C.seq0
      ~o_args:(OBus_value.C.seq1 OBus_value.C.basic_string)
      ()
  in
  try
    Lwt.return
      (OBus_introspect.input (Xmlm.make_input ~strip:true (`String (0, str))))
  with Xmlm.Error ((line, column), err) ->
    Lwt.fail
      (Failure
         (Printf.sprintf
            "OBus_proxy.introspect: invalid document, at line %d: %s" line
