...
type monitor =
  OBus_proxy.t -> OBus_name.interface -> Lwt_switch.t -> map signal Lwt.t
...
type group = {
  ...
  g_monitor : monitor; (* Monitor for this group. *)
}
...
type cache = {
  ...
  c_switch : Lwt_switch.t; (* Switch for the signal used to monitor the group. *)
}
...
let update_map context dict map =
  List.fold_left
    (fun map (name, value) -> String_map.add name (context, value) map)
    map dict

let map_of_list context dict = update_map context dict String_map.empty
  ...
  let%lwt event =
    OBus_signal.connect ~switch
      (OBus_signal.with_filters
         ...
         (OBus_signal.with_context (OBus_signal.make s_PropertiesChanged proxy)))
  and context, dict = get_all_no_cache proxy interface in
  Lwt.return
    (S.map snd
       (S.fold_s
          ~eq:(fun (_, a) (_, b) -> String_map.equal ( = ) a b)
...
let make ?(monitor = default_monitor) desc proxy =
  {
    ...
    p_cast =
      (fun context value ->
        OBus_value.C.cast_single (OBus_member.Property.typ desc) value);
    p_make = OBus_value.C.make_single (OBus_member.Property.typ desc);
  }

let group ?(monitor = default_monitor) proxy interface =
  { g_proxy = proxy; g_interface = interface; g_monitor = monitor }
...
let find_value_with_context name map = String_map.find name map
...
let get_with_context prop =
  ...
  | Some info -> (
      ...
      | None ->
          let%lwt context, value =
            OBus_method.call_with_context m_Get prop.p_proxy
              (prop.p_interface, prop.p_member)
          in
...
let get prop = get_with_context prop >|= snd

let set prop value =
  OBus_method.call m_Set prop.p_proxy
    (prop.p_interface, prop.p_member, prop.p_make value)
...
let get_group group =
  ...
  | Some info -> (
      ...
      | None ->
          let%lwt context, dict =
            get_all_no_cache group.g_proxy group.g_interface
          in
...
let finalise disable _ = ignore (Lazy.force disable)
...
let monitor_group ?switch group =
  ...
  let info =
    ...
    | None ->
        ...
        OBus_connection.set
          (OBus_proxy.connection group.g_proxy)
          key (Some info);
        info
  in
  let%lwt cache =
    match
      try Some (Group_map.find cache_key info.cache) with Not_found -> None
    with
    | Some cache_thread -> cache_thread
    | None -> (
        ...
        try%lwt
          let%lwt signal =
            group.g_monitor group.g_proxy group.g_interface switch
          in
          let cache = { c_count = 0; c_map = signal; c_switch = switch } in
        ...
        with exn ->
          let%lwt () =
            Lwt_log.warning_f ~section ~exn
              "failed to disable monitoring of properties for interface %S on \
               object %S from %S"
              group.g_interface
...
let monitor ?switch prop =
  let%lwt signal =
    monitor_group ?switch
      {
        g_interface = prop.p_interface;
        g_proxy = prop.p_proxy;
        g_monitor = prop.p_monitor;
      }
  in
