...
module Method = struct
  ...
  let make ~interface ~member ~i_args ~o_args ~annotations =
    { interface; member; i_args; o_args; annotations }
  ...
  let introspect m =
    Method
      ( m.member,
        introspect_arguments m.i_args,
        introspect_arguments m.o_args,
        m.annotations )
end
...
module Signal = struct
  ...
  let make ~interface ~member ~args ~annotations =
    { interface; member; args; annotations }
...
module Property = struct
  ...
  let make ~interface ~member ~typ ~access ~annotations =
    { interface; member; typ; access; annotations }
  ...
  let introspect p =
    Property
      ( p.member,
        OBus_value.C.type_single p.typ,
        ( match p.access with
        | Readable -> Read
        | Writable -> Write
