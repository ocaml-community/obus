...
struct
  ...
  let make_start_service_by_name_result = function
    | 2l -> `Already_running
    | 1l -> `Success
    | n -> Printf.ksprintf failwith "invalid value for \"start_service_by_name_result\": %ld" n
  ...
  let make_release_name_result = function
    ...
    | 2l -> `Non_existent
    | 1l -> `Released
    | n -> Printf.ksprintf failwith "invalid value for \"release_name_result\": %ld" n
  ...
  let make_request_name_result = function
    ...
    | 2l -> `In_queue
    | 1l -> `Primary_owner
    | n -> Printf.ksprintf failwith "invalid value for \"request_name_result\": %ld" n
  ...
  let m_ReleaseName = {
    ...
    Method.annotations = [(OBus_introspect_ext.obus_otype, "release_name_result")];
  }
  let m_ReloadConfig = {
    Method.interface = interface;
  ...
  let m_RequestName = {
    ...
    Method.o_args = (arg1
                       (Some "result", basic_uint32));
    Method.annotations = [(OBus_introspect_ext.obus_otype, "request_name_result");
  ...
  let m_StartServiceByName = {
    ...
    Method.o_args = (arg1
                       (Some "result", basic_uint32));
    Method.annotations = [(OBus_introspect_ext.obus_otype, "start_service_by_name_result")];
  }
  let m_UpdateActivationEnvironment = {
    Method.interface = interface;
    Method.member = "UpdateActivationEnvironment";
    Method.i_args = (arg1
                       (Some "x1", dict string basic_string));
    Method.o_args = (arg0);
    Method.annotations = [];
  }
  let s_NameAcquired = {
  ...
  let make members =
    OBus_object.make_interface_unsafe interface
      [
        (OBus_introspect_ext.obus_enum, "release_name_result:uint32=3:not_owner,2:non_existent,1:released");
        (OBus_introspect_ext.obus_enum, "request_name_result:uint32=4:already_owner,3:exists,2:in_queue,1:primary_owner");
        (OBus_introspect_ext.obus_enum, "start_service_by_name_result:uint32=2:already_running,1:success");
        (OBus_introspect_ext.obus_flag, "request_name_flags:uint32=4:do_not_queue,2:replace_existing,1:allow_replacement");
      ]
      [|
        method_info m_AddMatch members.m_AddMatch;
        method_info m_GetAdtAuditSessionData members.m_GetAdtAuditSessionData;
        method_info m_GetConnectionSELinuxSecurityContext members.m_GetConnectionSELinuxSecurityContext;
        method_info m_GetConnectionUnixProcessID members.m_GetConnectionUnixProcessID;
        ...
        method_info m_UpdateActivationEnvironment members.m_UpdateActivationEnvironment;
      |]
      [|
...
struct
  ...
  let m_GetAll = {
    ...
    Method.i_args = (arg1
                       (Some "interface_name", basic_string));
    Method.o_args = (arg1
                       (Some "values", dict string variant));
    Method.annotations = [];
  }
  let m_Set = {
    Method.interface = interface;
