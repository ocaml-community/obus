...
let make ?switch ~recv ~send ?(capabilities = []) ~shutdown () =
  let transport = { recv; send; capabilities; shutdown } in
...
let socket ?switch ?(capabilities = []) fd =
  let transport =
    if List.mem `Unix_fd capabilities then
      let reader = OBus_wire.reader fd and writer = OBus_wire.writer fd in
      {
        ...
        shutdown =
          (fun _ ->
            let%lwt () =
              OBus_wire.close_reader reader <&> OBus_wire.close_writer writer
            in
    ...
    send =
      (fun m ->
        Lwt_mvar.put mvar
          {
            m with
            OBus_message.body = OBus_value.V.sequence_dup (OBus_message.body m);
          });
...
let rec write_nonce fd nonce pos len =
  ...
  | 0 ->
      Lwt.fail
        (Failure
           "OBus_transport.connect: failed to send the nonce to the server")
  | n ->
      if n = len then Lwt.return () else write_nonce fd nonce (pos + n) (len - n)
...
let make_socket_nonce nonce_file domain typ addr =
  ...
  | None ->
      Lwt.fail
        (Invalid_argument
           "OBus_transport.connect: missing 'noncefile' parameter")
  | Some file_name ->
      let%lwt nonce =
        try%lwt
        ...
        | Unix.Unix_error (err, _, _) ->
            Lwt.fail
              (Failure
                 (Printf.sprintf "failed to read the nonce file '%s': %s"
                    file_name (Unix.error_message err)))
        | End_of_file ->
            Lwt.fail
              (Failure
                 (Printf.sprintf
                    "OBus_transport.connect: '%s' is an invalid nonce-file"
                    file_name))
      in
...
let rec connect address =
  ...
  | "unix" -> (
      ...
      | None, None, Some tmpd ->
          Lwt.fail
            (Invalid_argument
               "OBus_transport.connect: unix tmpdir can only be used as a \
                listening address")
      | _ ->
          Lwt.fail
            (Invalid_argument
  ...
  | ("tcp" | "nonce-tcp") as name -> (
      ...
      let opts =
        ...
        | Some family ->
            Printf.ksprintf invalid_arg
              "OBus_transport.connect: unknown address family '%s'" family
        | None -> opts
      in
      ...
      | [] ->
          Lwt.fail
            (Failure
               (Printf.sprintf
                  "OBus_transport.connect: no address info for host=%s \
                   port=%s%s"
                  host port
                  ( match OBus_address.arg "family" address with
                  | None -> ""
      ...
      | ai :: ais -> (
          ...
          in
          try%lwt make_socket ai.ai_family ai.ai_socktype ai.ai_addr
          with exn ->
  ...
  | "launchd" -> (
          ...
          let%lwt path =
            try%lwt
              Lwt_process.pread_line
                ("launchctl", [| "launchctl"; "getenv"; env |])
            with exn ->
      ...
      | None ->
          Lwt.fail
            (Invalid_argument
               "OBus_transport.connect: missing 'env' in launchd address") )
  | "autolaunch" -> (
      let%lwt addresses =
        ...
        in
        let line =
          try String.sub line 0 (String.index line '\000') with _ -> line
        in
        try%lwt Lwt.return (OBus_address.of_string line)
      ...
      | [] ->
          let%lwt () =
            Lwt_log.error_f ~section "'autolaunch' returned no addresses"
          in
          Lwt.fail (Failure "'autolaunch' returned no addresses")
      | address :: rest -> (
          try%lwt connect address
          with exn ->
            let rec find = function
              | [] -> Lwt.fail exn
              | address :: rest -> (
                  try%lwt connect address with exn -> find rest )
            in
            find rest ) )
  | name -> Lwt.fail (Failure ("unknown transport type: " ^ name))

let of_addresses ?switch ?(capabilities = OBus_auth.capabilities) ?mechanisms
    addresses =
  Lwt_switch.check switch;
  match addresses with
  | [] ->
      Lwt.fail
        (Invalid_argument "OBus_transport.of_addresses: no address given")
  | addr :: rest -> (
      (* Search an address for which connection succeed: *)
      let%lwt fd, domain =
        try%lwt connect addr
        with exn ->
          ...
          let rec find = function
            | [] ->
                ...
                Lwt.fail exn
            | addr :: rest -> ( try%lwt connect addr with exn -> find rest )
          in
          find rest
      in
      ...
      try%lwt
        ...
        | 0 ->
            Lwt.fail
              (OBus_auth.Auth_failure "failed to send the initial null byte")
        | 1 ->
            let%lwt guid, capabilities =
              OBus_auth.Client.authenticate
                ~capabilities:
                  (List.filter
                     (function `Unix_fd -> domain = PF_UNIX)
                     capabilities)
                ?mechanisms
