...
let hex_decode str =
  ...
  with Invalid_argument _ -> failwith "invalid hex-encoded data"
...
module Cookie = struct
  type t = { id : int32; time : int64; cookie : string }
...
module Keyring : sig
  ...
  let parse_line line =
    Scanf.sscanf line "%ld %Ld %[a-fA-F0-9]" (fun id time cookie ->
        { Cookie.id; Cookie.time; Cookie.cookie })

  let print_line cookie =
    sprintf "%ld %Ld %s" (Cookie.id cookie) (Cookie.time cookie)
  ...
  let load context =
    ...
    if Sys.file_exists fname then
      ...
      with exn ->
        let%lwt fname = keyring_file_name context in
        let%lwt () =
          Lwt_log.error_f ~exn ~section "failed to load cookie file %s" fname
        in
        Lwt.fail exn
    else Lwt.return []
  ...
  let lock_file fname =
    let really_lock () =
      Lwt_unix.openfile fname [ Unix.O_WRONLY; Unix.O_EXCL; Unix.O_CREAT ] 0o600
      >>= Lwt_unix.close
    in
    let rec aux = function
      | 0 -> (
            ...
            with Unix.Unix_error (error, _, _) as exn ->
              let%lwt () =
                Lwt_log.error_f ~section
                  "failed to remove stale lock file %s: %s" fname
          ...
          with Unix.Unix_error (error, _, _) as exn ->
            let%lwt () =
              Lwt_log.error_f ~section
                "failed to lock file %s after removing it: %s" fname
                (Unix.error_message error)
            in
            Lwt.fail exn )
      | n -> (
          ...
          with exn ->
            let%lwt () =
              Lwt_log.info_f ~section "waiting for lock file (%d) %s" n fname
            in
  ...
  let unlock_file fname =
    ...
    with Unix.Unix_error (error, _, _) as exn ->
      let%lwt () =
        Lwt_log.error_f ~section "failed to unlink file %s: %s" fname
          (Unix.error_message error)
      in
      Lwt.fail exn
  ...
  let save context cookies =
    ...
    let%lwt () =
      ...
      if not (Sys.file_exists dir) then
        try%lwt Lwt_unix.mkdir dir 0o700
        with Unix.Unix_error (error, _, _) as exn ->
          let%lwt () =
            Lwt_log.error_f ~section
              "failed to create directory %s with permissions 0600: %s" dir
              (Unix.error_message error)
          in
          Lwt.fail exn
      else Lwt.return ()
    in
    let%lwt () = lock_file lock_fname in
    (let%lwt () =
       try%lwt
       ...
       with exn ->
         let%lwt () =
           Lwt_log.error_f ~exn ~section
             "unable to write temporary keyring file %s" tmp_fname
         in
         Lwt.fail exn
     in
     try Lwt_unix.rename tmp_fname fname
     with Unix.Unix_error (error, _, _) as exn ->
       let%lwt () =
         Lwt_log.error_f ~section "unable to rename file %s to %s: %s" tmp_fname
           fname (Unix.error_message error)
       in
       Lwt.fail exn)
...
type stream = { recv : unit -> string Lwt.t; send : string -> unit Lwt.t }
...
let make_stream ~recv ~send =
  {
    recv =
      (fun () ->
        try%lwt recv () with
        | Auth_failure _ as exn -> Lwt.fail exn
        | End_of_file -> Lwt.fail (Auth_failure "input: premature end of input")
        | exn -> Lwt.fail (Auth_failure ("input: " ^ Printexc.to_string exn)));
    send =
      (fun line ->
        try%lwt send line with
        | Auth_failure _ as exn -> Lwt.fail exn
        | exn -> Lwt.fail (Auth_failure ("output: " ^ Printexc.to_string exn)));
  }
    ...
    ~recv:(fun () ->
      ...
      let rec loop last =
        if Buffer.length buf > max_line_length then
          ...
          | None -> Lwt.fail (Auth_failure "input: premature end of input")
          | Some ch ->
    ...
    ~recv:(fun () ->
      ...
      let rec loop last =
        if Buffer.length buf > max_line_length then
          ...
          | 0 -> Lwt.fail (Auth_failure "input: premature end of input")
          | 1 ->
    ...
    ~send:(fun line ->
      let rec loop ofs len =
        if len = 0 then Lwt.return ()
          ...
          | 0 -> Lwt.fail (Auth_failure "output: zero byte written")
          | n ->
              assert (n > 0 && n <= len);
              loop (ofs + n) (len - n)
      in
...
let rec recv_line stream =
  ...
  else Lwt.return (String.sub line 0 (len - 2))

let rec first f str pos =
  if pos = String.length str then pos
  else match f str.[pos] with true -> pos | false -> first f str (pos + 1)

let rec last f str pos =
  if pos = 0 then pos
  else match f str.[pos - 1] with true -> pos | false -> first f str (pos - 1)
...
let preprocess_line line =
  ...
  String.iter
    (function
      | '\x01' .. '\x7f' -> () | _ -> failwith "non-ascii characters in command")
    line;
  (* Extract the command *)
...
let client_recv =
  recv "client" (fun command args ->
      match command with
      | "REJECTED" -> Server_rejected (split args)
      | "OK" ->
          Server_ok
            ( try OBus_uuid.of_string args
              with _ -> failwith "invalid hex-encoded guid" )
...
let client_send chans cmd =
  send_line "client" chans
    ( match cmd with
    | Client_auth None -> "AUTH"
    | Client_auth (Some (mechanism, None)) -> sprintf "AUTH %s" mechanism
    | Client_auth (Some (mechanism, Some data)) ->
        sprintf "AUTH %s %s" mechanism (hex_encode data)
...
let server_send chans cmd =
  send_line "server" chans
    ( match cmd with
...
module Client = struct
  ...
  class virtual mechanism_handler =
    object
      ...
      method data (chall : data) =
        Lwt.return (Mech_error "no data expected for this mechanism")

      method abort = ()
    end

  type mechanism = { mech_name : string; mech_exec : unit -> mechanism_handler }
  ...
  class mech_dbus_cookie_sha1_handler =
    object
      method init = Lwt.return (Mech_continue (string_of_int (Unix.getuid ())))

      method data chal =
        let%lwt () =
          Lwt_log.debug_f ~section "client: dbus_cookie_sha1: chal: %s" chal
        in
        let context, id, chal =
          Scanf.sscanf chal "%[^/\\ \n\r.] %ld %[a-fA-F0-9]%!"
            (fun context id chal -> (context, id, chal))
        in
        let%lwt keyring = Keyring.load context in
        let cookie =
          try List.find (fun cookie -> cookie.Cookie.id = id) keyring
          with Not_found ->
        ...
        let resp =
          sprintf "%s %s" rand
            (hex_encode
               (OBus_util.sha_1
                  (sprintf "%s:%s:%s" chal rand cookie.Cookie.cookie)))
        in
        let%lwt () =
          Lwt_log.debug_f ~section "client: dbus_cookie_sha1: resp: %s" resp
        in
  ...
  let default_mechanisms =
    [ mech_external; mech_dbus_cookie_sha1; mech_anonymous ]
  ...
  let find_working_mech implemented_mechanisms available_mechanisms =
    let rec aux = function
      ...
      | { mech_name = name; mech_exec = f } :: mechs -> (
          ...
          | Some l when not (List.mem name l) -> aux mechs
          | _ -> (
              let mech = f () in
              try%lwt
                mech#init >>= function
                | Mech_continue resp ->
                    Lwt.return
                      (Transition
                         ( Client_auth (Some (name, Some resp)),
  ...
  let transition mechs state cmd =
    match state with
    | Waiting_for_data mech -> (
        match cmd with
        | Server_data chal -> (
            try%lwt
              ...
              | Mech_continue resp ->
                  Lwt.return
                    (Transition (Client_data resp, Waiting_for_data mech, mechs))
              | Mech_ok resp ->
                  Lwt.return
                    (Transition (Client_data resp, Waiting_for_ok, mechs))
              | Mech_error msg ->
                  Lwt.return
                    (Transition (Client_error msg, Waiting_for_data mech, mechs))
            with exn ->
              Lwt.return
                (Transition
                   ( Client_error (Printexc.to_string exn),
                     Waiting_for_data mech,
                     mechs )) )
        | Server_rejected am ->
            mech#abort;
        ...
        | Server_error _ ->
            mech#abort;
            Lwt.return (Transition (Client_cancel, Waiting_for_reject, mechs))
        | Server_ok guid ->
            mech#abort;
        ...
        | Server_agree_unix_fd ->
            ...
            Lwt.return
              (Transition
                 ( Client_error "command not expected here",
                   Waiting_for_data mech,
                   mechs )) )
    | Waiting_for_ok -> (
        match cmd with
        | Server_ok guid -> Lwt.return (Success guid)
        | Server_rejected am -> next mechs am
        | Server_data _ | Server_error _ ->
            Lwt.return (Transition (Client_cancel, Waiting_for_reject, mechs))
        | Server_agree_unix_fd ->
            Lwt.return
              (Transition
  ...
  let authenticate ?(capabilities = []) ?(mechanisms = default_mechanisms)
      ~stream () =
    let rec loop = function
      ...
      | Success guid ->
          let%lwt caps =
            if List.mem `Unix_fd capabilities then
              ...
              | Server_agree_unix_fd -> Lwt.return [ `Unix_fd ]
              | Server_error _ -> Lwt.return []
              | _ ->
      ...
      | Failure -> auth_failure "authentication failure"
    in
...
module Server = struct
  ...
  class mech_external_handler user_id =
    object
      inherit mechanism_handler
  ...
  class mech_dbus_cookie_sha1_handler =
    object
      ...
      method data resp =
        try%lwt
          let%lwt () =
            Lwt_log.debug_f ~section "server: dbus_cookie_sha1: resp: %s" resp
          in
          match state with
          | `State1 ->
              ...
              let keyring =
                List.filter
                  (fun { Cookie.time } ->
                    time <= cur_time && Int64.sub cur_time time <= 300L)
                  keyring
              in
              (* Find a working cookie *)
              let%lwt id, cookie =
                match keyring with
                | { Cookie.id; Cookie.cookie } :: _ ->
                ...
                | [] ->
                    ...
                    let%lwt () =
                      Keyring.save context
                        [ { Cookie.id; Cookie.time = cur_time; Cookie.cookie } ]
                    in
                    Lwt.return (id, cookie)
              in
              let rand = hex_encode (OBus_util.random_string 16) in
              ...
              let%lwt () =
                Lwt_log.debug_f ~section "server: dbus_cookie_sha1: chal: %s"
                  chal
              in
              state <- `State2 (cookie, rand);
              Lwt.return (Mech_continue chal)
          | `State2 (cookie, my_rand) ->
              Scanf.sscanf resp "%s %s" (fun its_rand comp_sha1 ->
                  if
                    OBus_util.sha_1 (sprintf "%s:%s:%s" my_rand its_rand cookie)
                    = hex_decode comp_sha1
                  then Lwt.return (Mech_ok user_id)
                  else Lwt.return Mech_reject)
        with _ -> Lwt.return Mech_reject
  ...
  let reject mechs =
    Lwt.return
      (Transition (Server_rejected (List.map mech_name mechs), Waiting_for_auth))

  let error msg = Lwt.return (Transition (Server_error msg, Waiting_for_auth))

  let transition user_id guid capabilities mechs state cmd =
    match state with
    | Waiting_for_auth -> (
        match cmd with
        | Client_auth None -> reject mechs
        | Client_auth (Some (name, resp)) -> (
            match
              OBus_util.find_map
                (fun m -> if m.mech_name = name then Some m.mech_exec else None)
                mechs
            with
            | None -> reject mechs
            | Some f -> (
                ...
                try%lwt
                  ...
                  | None, None ->
                      Lwt.return
                        (Transition (Server_data "", Waiting_for_data mech))
                  | Some chal, None ->
                      Lwt.return
                        (Transition (Server_data chal, Waiting_for_data mech))
                  | Some chal, Some rest -> reject mechs
                  | None, Some resp -> (
                      ...
                      | Mech_ok uid ->
                          Lwt.return
                            (Transition
                               (Server_ok guid, Waiting_for_begin (uid, [])))
                      | Mech_reject -> reject mechs )
                with exn -> reject mechs ) )
    ...
    | Waiting_for_data mech -> (
        match cmd with
        | Client_data "" ->
            Lwt.return (Transition (Server_data "", Waiting_for_data mech))
        | Client_data resp -> (
            try%lwt
              mech#data resp >>= function
              | Mech_continue chal ->
              ...
              | Mech_ok uid ->
                  Lwt.return
                    (Transition (Server_ok guid, Waiting_for_begin (uid, [])))
              | Mech_reject -> reject mechs
            with exn -> reject mechs )
        | Client_begin ->
            mech#abort;
        ...
        | _ ->
            mech#abort;
            error "DATA command expected" )
    | Waiting_for_begin (uid, caps) -> (
        match cmd with
        | Client_begin -> Lwt.return (Accept (uid, caps))
              ...
              Lwt.return
                (Transition
                   ( Server_agree_unix_fd,
                     Waiting_for_begin
                       ( uid,
                         if List.mem `Unix_fd caps then caps
                         else `Unix_fd :: caps ) ))
            else
              Lwt.return
                (Transition
                   ( Server_error
                       "Unix fd passing is not supported by this server",
                     Waiting_for_begin (uid, caps) ))
  ...
  let authenticate ?(capabilities = []) ?(mechanisms = default_mechanisms)
    ...
    let rec loop state count =
      ...
      | Transition (cmd, state) ->
          let count =
            match cmd with Server_rejected _ -> count + 1 | _ -> count
          in
          (* Specification do not specify a limit for rejected, so
             we choose one arbitrary *)
          if count >= max_reject then auth_failure "too many reject"
          else
            let%lwt () = server_send stream cmd in
            loop state count
      | Accept (uid, caps) -> Lwt.return (uid, caps)
      | Failure -> auth_failure "authentication failure"
    in
