  ...
  with exn ->
    ignore
      (Lwt_log.error_f ~section ~exn
         "failed to read the local machine uuid from file %S" file);
    Lwt.fail exn

let machine_uuid =
  lazy
    ( try%lwt read_uuid_file OBus_config.machine_uuid_file
      with exn -> (
        try%lwt read_uuid_file "/etc/machine-id" with _ -> Lwt.fail exn ) )
