...
module T = struct
  ...
  let rec print_single pp = function
    ...
    | Dict (tk, tv) ->
        fprintf pp "@[<2>T.Dict@,(@[<hv>%a,@ %a@])@]" print_basic tk
          print_single tv
  ...
  let rec aux_single length depth_struct depth_array depth_dict_entry = function
    ...
    | T.Dict (tk, tv) ->
        if depth_array > OBus_protocol.max_type_recursion_depth then
          ...
          aux_single (length + 4) depth_struct (depth_array + 1)
            (depth_dict_entry + 1) tv
    | T.Structure [] -> failwith "empty struct"
    | T.Structure tl ->
        if depth_struct > OBus_protocol.max_type_recursion_depth then
          failwith "too many nested structs"
        else
          aux_sequence (length + 2) (depth_struct + 1) depth_array
            depth_dict_entry tl
  and aux_sequence length depth_struct depth_array depth_dict_entry = function
    | [] -> if length > 255 then failwith "signature too long" else length
    | t :: tl ->
        aux_sequence
          (aux_single length depth_struct depth_array depth_dict_entry t)
...
let signature_length l =
  let rec aux_single length = function
    | T.Basic _ | T.Variant -> length + 1
    | T.Array t -> aux_single (length + 1) t
    | T.Dict (tk, tv) -> aux_single (length + 4) tv
    | T.Structure tl -> aux_sequence (length + 2) tl
  and aux_sequence length = function
    | [] -> length
    | t :: tl -> aux_sequence (aux_single length t) tl
  in
  aux_sequence 0 l
...
let signature_of_string str =
  ...
  let get_char () =
    let j = !i in
    if j = len then fail "premature end of signature"
    else (
      i := j + 1;
  ...
  let rec parse_single = function
    | 'a' -> (
        ...
        | '{' -> (
            ...
            | '}' -> T.Dict (tk, tv)
            | _ -> fail "'}' missing" )
        | ch -> T.Array (parse_single ch) )
    | '(' -> T.Structure (parse_struct (get_char ()))
    | ')' -> fail "')' without '('"
    | 'v' -> T.Variant
    | ch -> T.Basic (parse_basic "invalid type code: %c" ch)
  ...
  | Some msg -> invalid_signature str msg
  | None -> s
...
let string_of_signature signature =
  ...
  try
    ...
    str
  with Failure msg -> raise (Invalid_signature (str, msg))
...
module V = struct
      ...
      List.iter
        (function
          | Basic (Byte x) ->
              ...
              incr i
          | _ -> invalid_arg "OBus_value.array: unexpected type")
        l;
      ...
      List.iter
        (fun x ->
          if type_of_single x <> t then
            invalid_arg "OBus_value.array: unexpected type")
        l;
      Array (t, l) )
    ...
    List.iter
      (fun (k, v) ->
        if type_of_basic k <> tk || type_of_single v <> tv then
          invalid_arg "OBus_value.dict: unexpected type")
      l;
    Dict (tk, tv, l)

  let unsafe_array t l = if t = T.Basic T.Byte then array t l else Array (t, l)

  let unsafe_dict tk tv l = Dict (tk, tv, l)
  ...
  let rec print_single pp = function
    ...
    | Dict (tk, tv, l) ->
        print_list
          (fun pp (k, v) ->
            fprintf pp "(@[%a,@ %a@])" print_basic k print_single v)
          pp l
    | Structure l -> print_sequence pp l
    | Variant x ->
        fprintf pp "@[<2>Variant@,(@[<hv>%a,@ %a@])@]" T.print_single
          (type_of_single x) print_single x
  ...
  let basic_contains_fds = function T.Unix_fd -> true | _ -> false
  ...
  let rec single_collect_fds acc = function
    | Basic v -> basic_collect_fds acc v
    | Array (t, l) ->
        if single_contains_fds t then List.fold_left single_collect_fds acc l
        else acc
    | Dict (tk, tv, l) ->
        if basic_contains_fds tk || single_contains_fds tv then
          List.fold_left
            (fun acc (k, v) -> basic_collect_fds (single_collect_fds acc v) k)
            acc l
        else acc
    | Structure l -> sequence_collect_fds acc l
    | Variant v -> single_collect_fds acc v
    | Byte_array _ -> acc

  and sequence_collect_fds acc l = List.fold_left single_collect_fds acc l
    ...
    Lwt_list.iter_p
      (fun fd ->
        ...
        with Unix.Unix_error (err, _, _) ->
          Lwt_log.error_f ~section "failed to close file descriptor: %s"
  ...
  let basic_dup map = function
    | Unix_fd fd -> (
        try Unix_fd (FD_map.find fd !map)
        with Not_found ->
  ...
  let rec single_dup map = function
    | Basic x -> basic (basic_dup map x)
    | Array (t, l) as v ->
        if single_contains_fds t then array t (List.map (single_dup map) l)
        else v
    | Dict (tk, tv, l) as v ->
        if basic_contains_fds tk || single_contains_fds tv then
          dict tk tv
            (List.map (fun (k, v) -> (basic_dup map k, single_dup map v)) l)
        else v
    | Structure l -> structure (sequence_dup map l)
    | Byte_array _ as v -> v
    | Variant x -> variant (single_dup map x)

  and sequence_dup map l = List.map (single_dup map) l
...
module C = struct
  ...
  let dyn_basic t =
    {
      ...
      basic_cast =
        (fun x ->
          if V.type_of_basic x <> t then raise Signature_mismatch else x);
    }
  ...
  let dyn_single t =
    {
      ...
      single_cast =
        (fun x ->
          if V.type_of_single x <> t then raise Signature_mismatch else x);
    }
  ...
  let dyn_sequence t =
    {
      ...
      sequence_cast =
        (fun x ->
          if V.type_of_sequence x <> t then raise Signature_mismatch else x);
    }
  ...
  let byte =
    {
      basic_type = T.Byte;
      basic_make = V.byte;
      basic_cast = (function V.Byte x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_byte =
    {
      ...
      single_cast =
        (function V.Basic (V.Byte x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let boolean =
    {
      ...
      basic_cast = (function V.Boolean x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_boolean =
    {
      ...
      single_cast =
        (function V.Basic (V.Boolean x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let int16 =
    {
      basic_type = T.Int16;
      basic_make = V.int16;
      basic_cast = (function V.Int16 x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_int16 =
    {
      ...
      single_cast =
        (function V.Basic (V.Int16 x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let int32 =
    {
      basic_type = T.Int32;
      basic_make = V.int32;
      basic_cast = (function V.Int32 x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_int32 =
    {
      ...
      single_cast =
        (function V.Basic (V.Int32 x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let int64 =
    {
      basic_type = T.Int64;
      basic_make = V.int64;
      basic_cast = (function V.Int64 x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_int64 =
    {
      ...
      single_cast =
        (function V.Basic (V.Int64 x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let uint16 =
    {
      basic_type = T.Uint16;
      basic_make = V.uint16;
      basic_cast = (function V.Uint16 x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_uint16 =
    {
      ...
      single_cast =
        (function V.Basic (V.Uint16 x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let uint32 =
    {
      basic_type = T.Uint32;
      basic_make = V.uint32;
      basic_cast = (function V.Uint32 x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_uint32 =
    {
      ...
      single_cast =
        (function V.Basic (V.Uint32 x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let uint64 =
    {
      basic_type = T.Uint64;
      basic_make = V.uint64;
      basic_cast = (function V.Uint64 x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_uint64 =
    {
      ...
      single_cast =
        (function V.Basic (V.Uint64 x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let double =
    {
      basic_type = T.Double;
      basic_make = V.double;
      basic_cast = (function V.Double x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_double =
    {
      ...
      single_cast =
        (function V.Basic (V.Double x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let string =
    {
      basic_type = T.String;
      basic_make = V.string;
      basic_cast = (function V.String x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_string =
    {
      ...
      single_cast =
        (function V.Basic (V.String x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let signature =
    {
      ...
      basic_cast =
        (function V.Signature x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_signature =
    {
      ...
      single_cast =
        (function
        | V.Basic (V.Signature x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let object_path =
    {
      ...
      basic_cast =
        (function V.Object_path x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_object_path =
    {
      ...
      single_cast =
        (function
        | V.Basic (V.Object_path x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let unix_fd =
    {
      ...
      basic_cast = (function V.Unix_fd x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic_unix_fd =
    {
      ...
      single_cast =
        (function V.Basic (V.Unix_fd x) -> x | _ -> raise Signature_mismatch);
    }
  ...
  let basic t =
    {
      ...
      single_cast =
        (function V.Basic x -> t.basic_cast x | _ -> raise Signature_mismatch);
    }
  ...
  let structure t =
    {
      ...
      single_cast =
        (function
        | V.Structure x -> t.sequence_cast x | _ -> raise Signature_mismatch);
    }
  ...
  let byte_array =
    {
      ...
      single_cast =
        (function V.Byte_array x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let array t =
    {
      ...
      single_cast =
        (function
        | V.Array (t', x) when t.single_type = t' -> List.map t.single_cast x
        | V.Byte_array s when t.single_type = T.basic_byte ->
            let rec aux acc = function
              | -1 -> acc
              | i ->
                  aux
                    (t.single_cast (V.basic_byte (String.unsafe_get s i)) :: acc)
                    (i - 1)
            in
      ...
      single_make =
        (fun x ->
          V.Dict
            ( tk.basic_type,
              tv.single_type,
              List.map (fun (k, v) -> (tk.basic_make k, tv.single_make v)) x ));
      single_cast =
        (function
  ...
  let variant =
    {
      ...
      single_cast =
        (function V.Variant x -> x | _ -> raise Signature_mismatch);
    }
  ...
  let seq0 =
    {
      sequence_type = [];
      sequence_make = (fun () -> []);
      sequence_cast = (function [] -> () | _ -> raise Signature_mismatch);
    }
  ...
  let seq1 t1 =
    {
      ...
      sequence_cast =
        (function [ x1 ] -> t1.single_cast x1 | _ -> raise Signature_mismatch);
    }
  ...
  let seq3 t1 t2 t3 =
    {
      ...
      sequence_make =
        (fun (x1, x2, x3) ->
          [ t1.single_make x1; t2.single_make x2; t3.single_make x3 ]);
      sequence_cast =
        (function
        | [ x1; x2; x3 ] ->
            (t1.single_cast x1, t2.single_cast x2, t3.single_cast x3)
        | _ -> raise Signature_mismatch);
    }
  ...
  let seq4 t1 t2 t3 t4 =
    {
      sequence_type =
        [ t1.single_type; t2.single_type; t3.single_type; t4.single_type ];
      sequence_make =
        (fun (x1, x2, x3, x4) ->
          [
            t1.single_make x1;
...
let arguments ~arg_types ~arg_names =
  if List.length arg_types.C.sequence_type = List.length arg_names then
    { arg_types; arg_names }
  else invalid_arg "OBus_value.arguments"
...
let arg_cons (name, t) args =
  { arg_types = seq_cons t args.arg_types; arg_names = name :: args.arg_names }

let arg0 = { arg_types = seq0; arg_names = [] }

let arg1 (n1, t1) = { arg_types = seq1 t1; arg_names = [ n1 ] }

let arg2 (n1, t1) (n2, t2) = { arg_types = seq2 t1 t2; arg_names = [ n1; n2 ] }

let arg3 (n1, t1) (n2, t2) (n3, t3) =
  { arg_types = seq3 t1 t2 t3; arg_names = [ n1; n2; n3 ] }

let arg4 (n1, t1) (n2, t2) (n3, t3) (n4, t4) =
  { arg_types = seq4 t1 t2 t3 t4; arg_names = [ n1; n2; n3; n4 ] }

let arg5 (n1, t1) (n2, t2) (n3, t3) (n4, t4) (n5, t5) =
  { arg_types = seq5 t1 t2 t3 t4 t5; arg_names = [ n1; n2; n3; n4; n5 ] }

let arg6 (n1, t1) (n2, t2) (n3, t3) (n4, t4) (n5, t5) (n6, t6) =
  { arg_types = seq6 t1 t2 t3 t4 t5 t6; arg_names = [ n1; n2; n3; n4; n5; n6 ] }
...
let arg9 (n1, t1) (n2, t2) (n3, t3) (n4, t4) (n5, t5) (n6, t6) (n7, t7) (n8, t8)
    (n9, t9) =
  {
...
let arg10 (n1, t1) (n2, t2) (n3, t3) (n4, t4) (n5, t5) (n6, t6) (n7, t7)
    (n8, t8) (n9, t9) (n10, t10) =
  {
...
let arg16 (n1, t1) (n2, t2) (n3, t3) (n4, t4) (n5, t5) (n6, t6) (n7, t7)
    (n8, t8) (n9, t9) (n10, t10) (n11, t11) (n12, t12) (n13, t13) (n14, t14)
    (n15, t15) (n16, t16) =
  {
