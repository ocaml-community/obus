...
let validate str =
  let fail i msg = Some { typ = "path"; str; ofs = i; msg }
  and len = length str in
  let rec aux_element_start i =
    if i = len then fail (i - 1) "trailing '/'"
    else if is_valid_char (unsafe_get str i) then aux_element (i + 1)
    else if unsafe_get str i = '/' then fail i "empty element"
    else fail i "invalid char"
  and aux_element i =
    if i = len then None
    else
      let ch = unsafe_get str i in
      if ch = '/' then aux_element_start (i + 1)
      else if is_valid_char ch then aux_element (i + 1)
      else fail i "invalid char"
  in
  if len = 0 then fail (-1) "empty path"
  else if unsafe_get str 0 = '/' then
    if len = 1 then None else aux_element_start 1
  else fail 0 "must start with '/'"
...
let validate_element = function
  ...
  | str ->
      ...
      let rec aux i =
        if i = len then None
        else if is_valid_char (unsafe_get str i) then aux (i + 1)
        else
          Some
            {
              typ = "path element";
...
let to_string = function
  ...
  | path ->
      let str =
        Bytes.create
          (List.fold_left (fun len elt -> len + length elt + 1) 0 path)
      in
      ignore
        (List.fold_left
           (fun pos elt ->
...
let unescape s =
  ...
  for i = 0 to len - 1 do
    ...
    Bytes.set r i
      (char_of_int
         ( (int_of_char s.[j] - int_of_char 'a')
         lor ((int_of_char s.[j + 1] - int_of_char 'a') lsl 4) ))
  done;
  Bytes.unsafe_to_string r

let rec after prefix path =
...
let generate () =
  ...
  let id2 = id2 + 1 in
  if id2 < 0 then unique_id := (id1 + 1, 0) else unique_id := (id1, id2);
