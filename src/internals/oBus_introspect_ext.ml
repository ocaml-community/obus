...
let check_values func typ values =
  match typ with
  | T.Byte | T.Int16 | T.Int32 | T.Int64 | T.Uint16 | T.Uint32 | T.Uint64 ->
      List.iter
        (fun (value, name) ->
...
let tuple = function [ t ] -> t | l -> Tuple l
...
let rec resolve env = function
  | Term (name, args) -> (
      ...
      | Some (Sym_enum (typ, values)) -> Basic (Enum (typ, values))
      | Some (Sym_flag (typ, values)) -> Basic (Flag (typ, values))
      | None -> (
          ...
          | "dict", [ tk; tv ] ->
              raise
                (Resolve_error "type of a dictionary key must be a basic type")
          | "variant", [] -> Variant
          | _ ->
              raise
                (Resolve_error
                   (sprintf "unbounded symbol: %S with arity %d" name
                      (List.length args))) ) )
  | Tuple l -> Structure (List.map (resolve env) l)
...
let rec string_of_term = function
  ...
  | Tuple typs -> "<" ^ String.concat "," (List.map string_of_term typs) ^ ">"
    ...
    (String.concat ","
       (List.map
          (fun (key, value) ->
            sprintf "%s:%s"
              ( match key with
              | V.Byte x -> string_of_int (Char.code x)
              | V.Int16 x | V.Uint16 x -> string_of_int x
              | V.Int32 x | V.Uint32 x -> Int32.to_string x
              | V.Int64 x | V.Uint64 x -> Int64.to_string x
              | _ -> assert false )
              value)
          values))
...
let term_of_string str =
  try OBus_type_ext_lexer.single (Lexing.from_string str)
  with OBus_type_ext_lexer.Fail (pos, msg) ->
    ksprintf failwith "failed to parse extended type %S, at position %d: %s" str
      pos msg

let enum_of_string str =
  try OBus_type_ext_lexer.enum_and_flag (Lexing.from_string str)
  with OBus_type_ext_lexer.Fail (pos, msg) ->
    ksprintf failwith "failed to parse extended symbol %S, at position %d: %s"
      str pos msg
...
let set_annotation name value annotations =
  let rec loop acc = function
    | [] -> (name, value) :: acc
    | (name', _) :: rest when name = name' ->
        (name, value) :: List.rev_append acc rest
...
let encode_arguments env args annotation_name annotations =
  let rec loop acc use_ext = function
    ...
    | (name, typ) :: rest ->
        ...
        else loop ((name, std_typ) :: acc) true rest
  in
  let args', use_ext = loop [] false args in
  if use_ext then
    ( args',
      set_annotation annotation_name
        (string_of_term (tuple (List.map snd args)))
        annotations )
    ...
    List.map
      (function
        | Method (name, i_args, o_args, annotations) ->
            let i_args, annotations =
              encode_arguments env i_args obus_itype annotations
            in
            let o_args, annotations =
              encode_arguments env o_args obus_otype annotations
            in
            OBus_introspect.Method (name, i_args, o_args, annotations)
        | Signal (name, args, annotations) ->
            let args, annotations =
              encode_arguments env args obus_type annotations
            in
            OBus_introspect.Signal (name, args, annotations)
        | Property (name, typ, access, annotations) -> (
            match
              encode_arguments env [ (None, typ) ] obus_type annotations
            with
...
let get_annotation name annotations =
  let rec loop acc = function
    | [] -> (acc, None)
    | (name', value) :: rest ->
        if name = name' then (List.rev_append acc rest, Some value)
...
let decode_arguments args annotation_name annotations =
  ...
  | annotations, Some value ->
      ( List.map2
          ...
          args
          (match term_of_string value with Tuple l -> l | t -> [ t ]),
        annotations )
  ...
  let members =
    List.map
      (function
        | OBus_introspect.Method (name, i_args, o_args, annotations) ->
            let i_args, annotations =
              decode_arguments i_args obus_itype annotations
            in
            let o_args, annotations =
              decode_arguments o_args obus_otype annotations
            in
            Method (name, i_args, o_args, annotations)
        | OBus_introspect.Signal (name, args, annotations) ->
            let args, annotations =
              decode_arguments args obus_type annotations
            in
            Signal (name, args, annotations)
        | OBus_introspect.Property (name, typ, access, annotations) -> (
