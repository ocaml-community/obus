...
let rec find_map f = function
  | [] -> None
  | x :: l -> ( match f x with None -> find_map f l | y -> y )

let filter_map f l =
  List.fold_right
    (fun x acc -> match f x with None -> acc | Some v -> v :: acc)
    l []

let part_map f l =
  List.fold_right
    (fun x (success, failure) ->
      match f x with
      | None -> (success, x :: failure)
      | Some v -> (v :: success, failure))
    l ([], [])
...
let split f l =
  List.fold_right
    (fun x (a, b) ->
      match f x with InL x -> (x :: a, b) | InR x -> (a, x :: b))
    l ([], [])

let map_option x f = match x with Some x -> Some (f x) | None -> None

let encode_char n =
  if n < 10 then char_of_int (n + Char.code '0')
  else if n < 16 then char_of_int (n + Char.code 'a' - 10)
  else assert false
...
let hex_decode hex =
  if String.length hex mod 2 <> 0 then
    raise (Invalid_argument "OBus_util.hex_decode");
  let len = String.length hex / 2 in
  let str = Bytes.create len in
  for i = 0 to len - 1 do
    Bytes.unsafe_set str i
      (char_of_int
         ( (decode_char (String.unsafe_get hex (i * 2)) lsl 4)
         lor decode_char (String.unsafe_get hex ((i * 2) + 1)) ))
  done;
  Bytes.unsafe_to_string str

let homedir =
  lazy
    ( try Lwt.return (Sys.getenv "HOME")
      with Not_found ->
  ...
  with exn ->
    ignore
      (Lwt_log.warning_f ~exn ~section
         "failed to get random data from /dev/urandom");
...
let sha_1 s =
  ...
  let d = ref 0l in
  let e = ref 0l in
  for i = 0 to (String.length m / 64) - 1 do
    (* For each block *)
    (* Fill w *)
    let base = i * 64 in
    for j = 0 to 15 do
      let k = base + (j * 4) in
      w.(j) <-
        sl (Int32.of_int (Char.code m.[k])) 24
        lor sl (Int32.of_int (Char.code m.[k + 1])) 16
        lor sl (Int32.of_int (Char.code m.[k + 2])) 8
        lor Int32.of_int (Char.code m.[k + 3])
    done;
    (* Loop *)
    a := !h0;
    b := !h1;
