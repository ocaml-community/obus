...
let () =
  Printexc.register_printer (function
    | Parse_failure ((line, column), msg) ->
        Some
          (Printf.sprintf
             "failed to parse D-Bus introspection document, at line %d, column \
              %d: %s"
             line column msg)
    | _ -> None)
...
let atype p =
  ...
  | [ t ] -> t
  | _ ->
      Printf.ksprintf (failwith p)
        "this signature contains more than one single type: %S" signature
...
let method_decl =
  elt "method" (fun p ->
      ...
      let ins, outs =
        OBus_util.split
          (function In, x -> OBus_util.InL x | Out, x -> OBus_util.InR x)
          args
      in
...
let property_decl =
  elt "property" (fun p ->
      ...
      let access =
        afr p "access"
          [ ("read", Read); ("write", Write); ("readwrite", Read_write) ]
      in
      let typ = atype p in
      let annots = annotations p in
...
let node =
  elt "node" (fun p ->
      ...
      | Some error ->
          failwith p
            (OBus_string.error_message
               { error with OBus_string.typ = "node name" }))

let interface =
  elt "interface" (fun p ->
      let name = ainterface p in
      let decls = any p (union [ method_decl; signal_decl; property_decl ]) in
      let annots = annotations p in
      (name, decls, annots))
...
let to_xml (ifaces, nodes) =
  let pannots =
    List.map (fun (n, v) ->
        Element ("annotation", [ ("name", n); ("value", v) ], []))
  in
  let pargs dir =
    List.map (fun (n, t) ->
        ...
        in
        let attrs =
          match n with Some n -> ("name", n) :: attrs | None -> attrs
        in
        Element ("arg", attrs, []))
  in
  Element
                  ...
                  (function
                    | Method (name, ins, outs, annots) ->
                        Element
                          ( "method",
                            [ ("name", name) ],
                            pargs (Some In) ins @ pargs (Some Out) outs
                            @ pannots annots )
                    | Signal (name, args, annots) ->
                        Element
                          ( "signal",
                            [ ("name", name) ],
                            pargs None args @ pannots annots )
                    | Property (name, typ, access, annots) ->
                        Element
                          ( "property",
                            [
...
let output xo doc =
  let rec aux (Element (name, attrs, children)) =
    Xmlm.output xo
      (`El_start
        (("", name), List.map (fun (name, value) -> (("", name), value)) attrs));
