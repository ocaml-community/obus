...
let error_message error =
  if error.ofs < 0 then
    ...
    Printf.sprintf "invalid D-Bus %s (%S), at position %d: %s" error.typ
      error.str error.ofs error.msg

let () =
  Printexc.register_printer (function
    | Invalid_string error -> Some (error_message error)
    | _ -> None)
...
let validate s =
  let fail i msg = Some { typ = "string"; str = s; ofs = i; msg } in
  ...
  let rec main i =
    if i = len then None
      ...
      match ch with
      | '\x00' -> fail i "null byte"
      | '\x01' .. '\x7f' -> main (i + 1)
      | '\xc0' .. '\xdf' ->
          if i + 1 >= len then fail len "premature end of UTF8 sequence"
          else
            let byte1 = Char.code (String.unsafe_get s (i + 1)) in
            if byte1 land 0xc0 != 0x80 then
              fail (i + 1) "malformed UTF8 sequence"
            else if
              ((Char.code ch land 0x1f) lsl 6) lor (byte1 land 0x3f) < 0x80
            then fail i "overlong UTF8 sequence"
            else main (i + 2)
      | '\xe0' .. '\xef' ->
          if i + 2 >= len then fail len "premature end of UTF8 sequence"
          else
      ...
      | '\xf0' .. '\xf7' ->
          if i + 3 >= len then fail len "premature end of UTF8 sequence"
          else
            let byte1 = Char.code (String.unsafe_get s (i + 1))
      ...
      | _ -> fail i "invalid start of UTF8 sequence"
  in
  main 0

let assert_validate validator str =
  match validator str with
