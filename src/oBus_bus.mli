...
type request_name_result =
  [ `Primary_owner  (** You are now the primary owner of the connection *)
  | `In_queue  (** You will get the name when it will be available *)
  | `Exists
    (** Somebody else already have the name and nobody specified
            what to do in this case *)
  | `Already_owner  (** You already have the name *) ]
...
val start_service_by_name :
  t -> OBus_name.bus -> start_service_by_name_result Lwt.t
...
val name_owner_changed :
  t -> (OBus_name.bus * OBus_name.bus * OBus_name.bus) OBus_signal.t
