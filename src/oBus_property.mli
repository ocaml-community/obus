...
type monitor =
  OBus_proxy.t -> OBus_name.interface -> Lwt_switch.t -> map React.signal Lwt.t
...
val map_rw_with_context :
  (OBus_context.t -> 'a -> 'b) -> ('b -> 'a) -> 'a rw -> 'b rw
...
val map_r_with_context :
  (OBus_context.t -> 'a -> 'b) -> ('a, [> `readable ]) t -> 'b r
...
val find_value_with_context :
  OBus_name.member -> map -> OBus_context.t * OBus_value.V.single
...
val monitor :
  ?switch:Lwt_switch.t -> ('a, [> `readable ]) t -> 'a React.signal Lwt.t
...
val update_map :
  OBus_context.t -> (OBus_name.member * OBus_value.V.single) list -> map -> map
(** [update_map context values map] add all properties with their
      context and value to [map]. *)

val map_of_list :
  OBus_context.t -> (OBus_name.member * OBus_value.V.single) list -> map
