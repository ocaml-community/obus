  ...
  | [] ->
      if tty then
        if failures = 0 then
          ...
          Lwt_io.printlf "\027[31;1m%d of %d tests failed.\027[0m" failures
            total
      else if failures = 0 then Lwt_io.printl "All tests succeeded!"
      else Lwt_io.printlf "%d of %d tests failed." failures total
  | (name, test) :: rest -> (
      let%lwt () = title name in
      ( try%lwt test ()
        with exn ->
          let%lwt () =
            Lwt_io.printlf "test failed with: %s" (Printexc.to_string exn)
          in
