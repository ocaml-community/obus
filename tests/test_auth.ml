...
let test_mech mech =
  try%lwt
      ...
      Lwt.join
        [
          (let%lwt _ =
           ...
           return ());
          (let%lwt _ =
             OBus_auth.Server.authenticate ~user_id ~mechanisms:[ mech ] ~guid
    ...
    in
    let%lwt () =
      Lwt_io.printlf "authentication %s works!"
        (OBus_auth.Server.mech_name mech)
    in
    return true
  with exn ->
    let%lwt () =
      Lwt_io.printlf "authentication %s do not works: %s"
        (OBus_auth.Server.mech_name mech)
